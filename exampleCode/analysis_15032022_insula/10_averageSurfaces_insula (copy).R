...
for ( i in 1:length(surfaceFile) ) { # i <- 92
...
surfaceData[,10] <- averageLH_thickness 
surfaceData[,11] <- averageLH_curvature
write.table( surfaceData, file=sprintf( '%s/average_insulaClusters_%s_vonEcomo_selectedParticipants_corrected_lh.1D.dset', atlasDir, filenameIn ), row.names=FALSE, col.names=FALSE )
...
surfaceData[,10] <- averageRH_thickness 
surfaceData[,11] <- averageRH_curvature
write.table( surfaceData, file=sprintf( '%s/average_insulaClusters_%s_vonEcomo_selectedParticipants_corrected_rh.1D.dset', atlasDir, filenameIn ), row.names=FALSE, col.names=FALSE )
