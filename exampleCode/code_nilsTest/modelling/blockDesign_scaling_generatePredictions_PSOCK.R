...
generatePrediction <- function( indexPrediction, inputPredictionGrid, samplingTime, mriTime, scaleData, stimData ) {
  ...
  #linear scaling - linear amplitude modulation over blocks
  nBlocks <- dim(stimData)[1]
  linearCoefficients <- round( seq(0.1, 1, length.out = nBlocks), 3 )
  
  # simple block design and linear amplitude modulation over blocks
  stimSequence <- rep(0,length(mriTime)) 
  amplitudeModulationLinear <- rep(0,length(mriTime))
  for (nStimuli in 1:dim(stimData)[1] ) { #nStimuli <- 1
    ...
    amplitudeModulationLinear[ idxStimuliOn:(idxStimuliOn+stimuliLength-1) ] <- linearCoefficients[nStimuli]
  }
  ...
  if ( sum( is.na( stimSequence ) ) == 0 ) {
    ...
    pConv02 <- conv( amplitudeModulationLinear, hrf )
    pConvTrim02 <- pConv02[ 1 : length( mriTime ) ]; 
    
    returnPrediction01 <- round( scaleData( pConvTrim01, 1, 0 ), 6 ); 
    returnPrediction02 <- round( scaleData( pConvTrim02, 1, 0 ), 6 ); 
    
    returnPrediction <- cbind( returnPrediction01, returnPrediction02 )
  ...
  if (testPlot==1) {
    ...
    plot( amplitudeModulationLinear ~ mriTime, type='l' )
    plot( pConvTrim01 ~ mriTime,type='l')
    plot( pConvTrim02 ~ mriTime,type='l')
    plot( returnPrediction01 ~ mriTime,type='l' )
    plot( returnPrediction02 ~ mriTime,type='l' )
  }
  
  if ( sum( is.na( stimSequence ) ) > 0 | sum( is.na( amplitudeModulationLinear ) ) > 0 ) {
    returnPrediction <- cbind( rep(0,length(mriTime)), rep(0,length(mriTime)) )
  }
...
predictionWrapper <- function( passIdx, limitsMat, inputGrid ) {
  ...
  if (passIdx<=9) {
    save( outMatrix, file = sprintf('%s/ttt_predictionFile_0%1.0f.RData', outDirectory, passIdx ) )
  }
  if (passIdx>=10 & passIdx<=99 ) {
