...
for ( counter in 1:(length(limits)-1) ) {
  ...
  instr <- sprintf( 'cp clip_0%1.0f_thr.nii.gz volumeN3.nii.gz', counter )
  system( instr )


  instr <- sprintf('3dSeg -anat volumeN3.nii.gz -mask maskVolume.nii.gz -classes \u0027CSF ; GM ; WM\u0027 -bias_classes \u0027GM ; WM\u0027 -bias_fwhm 15 -mixfrac UNI -main_N %s -blur_meth BFT', args[7] ) 
  print( instr )
  system( instr )
