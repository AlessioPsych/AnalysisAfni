...
for ( nSession in 1:length( epiSessions ) ) { # nSession <- 1
  ...
  dfTemp <- data.frame( anatTemp, sesTemp, sessionTemp, lengthEpiFilesTemp, fileIdxTemp )
...
for ( nCondition in 1:length( expConditions ) ) { # nCondition <- 1 # length( expConditions )
  ...
  for ( nPart in 1:length( participantsTemp ) ) { # nPart <- 1
    ...
    for ( nReps in 1:dim( partDfTemp )[1] ) { # nReps <- 1
      ...
      instrBeginning <- paste( instrBeginning,
                                sprintf( '%s/%s/epiOnAnat/%s', epiDir, partDfTemp[nReps,3], epiFiles[ partDfTemp[nReps,5] ]  ) )
    }
