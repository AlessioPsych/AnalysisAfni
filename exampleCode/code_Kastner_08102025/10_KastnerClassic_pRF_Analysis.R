...
toolboxDir <- Sys.getenv( x='AFNI_TOOLBOXDIR' ) 
source( sprintf('%s/scaleData.R', generalPurposeDir) )
source( sprintf('%s/AFNIio.R', afniInstallDir ) )
source( sprintf('%s/coregistration/computeDistanceMap.R', toolboxDir ) )
source( sprintf('%s/surfaces/coordinateFromLinearIndex.r', toolboxDir ) )
library( pracma )
library( abind )
library( circular )
library(RANN)
...
dirToCheck <- sprintf('%s/pRF_test', modelsDir )
...
for ( nPart in 1:3 ) { # nPart <- 1 1:length(pRFParticipants) length(pRFParticipants)
  ...
  dirToCheck <- sprintf('%s/pRF_test/%s', modelsDir, participantName )
  if ( dir.exists( dirToCheck ) ) {
    instr <- sprintf( 'rm -R %s', dirToCheck ) 
    print( instr )
    if (runCodeFlag==1) { system( instr ) }  
  ...
  instr <- paste('3dresample',
                 ...
                 sprintf( '-prefix %s/%s/%s/%s.zzz.lh.ribbon.nii', modelsDir, 'pRF_test', participantName, selectedAnatFolders[nPart] ), 
                 '-rmode NN' )
  print( instr )
  if (runCodeFlag==1) { system( instr ) }  
  ...
  instr <- paste('3dresample',
                 ...
                 sprintf( '-master %s', epiPrfTemp ),  
                 sprintf( '-prefix %s/%s/%s/%s.zzz.rh.ribbon.nii', modelsDir, 'pRF_test', participantName, selectedAnatFolders[nPart] ), 
                 '-rmode NN' )
  print( instr )
  if (runCodeFlag==1) { system( instr ) }  
  ...
  # combine ribbons
  instr <- paste('3dcalc', 
                 sprintf( '-a %s/%s/%s/%s.zzz.lh.ribbon.nii', modelsDir, 'pRF_test', participantName, selectedAnatFolders[nPart] ), 
                 sprintf( '-b %s/%s/%s/%s.zzz.rh.ribbon.nii', modelsDir, 'pRF_test', participantName, selectedAnatFolders[nPart] ), 
                 sprintf('-expr \u0027a+b\u0027'),
                 sprintf( '-prefix %s/%s/%s/%s.zzz.grayMatter.nii', modelsDir, 'pRF_test', participantName, selectedAnatFolders[nPart] ) ) 
  print( instr )
  if (runCodeFlag==1) { system( instr ) } 
  ...
  instr <- paste('cp',
                 ...
                 sprintf( '%s/%s/%s/%s', modelsDir, 'pRF_test', participantName, pRFParticipants[nPart] )
  )
  print( instr )
  if (runCodeFlag==1) { system( instr ) } 
  ...
  #print( instr )
  #if (runCodeFlag==1) { system( instr ) }
  #instr <- sprintf('3dcalc -a %s_ANATOMY.zzz.grayMatter.nii -expr \u0027a+1\u0027 -prefix %s_mask_auto01.nii.gz', participantName, participantName )  
  #print( instr )
  #if (runCodeFlag==1) { system( instr ) }
  #instr <- sprintf('3dmerge -1blur_fwhm 2 -doall -prefix %s_mask_auto02.nii.gz %s_mask_auto01.nii.gz', participantName, participantName )  
  #print( instr )
  #if (runCodeFlag==1) { system( instr ) }
  #computeDistanceMap( sprintf( '%s_ANATOMY.zzz.grayMatter.nii', participantName ) )
  #instr <- sprintf('3dcalc -a ttt_distanceMap_overall+orig -expr \u0027step(abs(a)-1.5)\u0027 -prefix %s_mask_auto01.nii.gz', participantName )  
  #print( instr )
  #if (runCodeFlag==1) { system( instr ) }

  instr <- sprintf('3dmask_tool -input %s_ANATOMY.zzz.grayMatter.nii -prefix %s_mask_auto.nii.gz -dilate_input 1 -1', participantName, participantName )  
  print( instr )
  if (runCodeFlag==1) { system( instr ) }
