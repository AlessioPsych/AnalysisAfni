...
#rm(list=ls())
#setwd('/data1/projects/myelin/myelinData/hemiBackup/hemianopticdata/V6002.hemianoptic')
#args <- c('pdCorrectRegularT1_noBlur_stripped.nii','/packages/afni/17.0.13','/data1/projects/myelin/analysisAfni/surfaces','/packages/afni/17.0.13','0.001','0.07','10','6-6-6-6-6-6','/data1/projects/myelin/analysisAfni/coregistration','0','1','-1')
...
#parameterWm <- as.numeric( strsplit( args[13], '[-]' )[[1]] ) 
...
limitsIS <- round( seq( infLimit, supLimit, length.out = 4  ), 0 )
#flagWmProcess <- parameterWm
...
if (parametersInput==0) {
  ...
  parameterSegmentationClass <- array(0,c(3,6))
}

for ( counter in c(1,2,3,4,5,6) ) {
  #for ( counter in 1:2 ) {
  setwd(mainDir)
  if (counter==1) {
  ...
  #instr <- sprintf( '3dCM clip_0%1.0f.nii.gz > _ttt_cm.1D', counter )
  #system( instr )
  #CMvalues <- read.table( '_ttt_cm.1D' )
  #CMvalue <- CMvalues[ 3  ]
  #system('rm _ttt_cm.1D')
  #instr <- sprintf( '@clip_volume -input clip_0%1.0f.nii.gz -inferior %1.0f -prefix clip_0%1.0f_clip.nii.gz', counter, CMvalue, counter )
  #system( instr )
  #system(sprintf('rm clip_0%1.0f.nii.gz',counter))
  #system(sprintf('mv clip_0%1.0f_clip.nii.gz clip_0%1.0f.nii.gz',counter,counter))
  ...
  instr <- sprintf('3dSeg -anat volumeN3.nii.gz -mask maskVolume.nii.gz -classes \u0027CSF ; GM ; WM\u0027 -bias_classes \u0027GM ; WM\u0027 -mixfrac UNI -main_N %s -blur_meth BFT', args[7] ) 
  print( instr )
  system( instr )
  ...
  if (parameterSegmentationClass[1,counter]==0) { classWM <- 3 }
  if (parameterSegmentationClass[1,counter]!=0) { classWM <- parameterSegmentationClass[2,counter] }
  
  if (parameterSegmentationProp[1,counter]==0) {
    emptyVol <- array(0, dim( classData3d ) )
    emptyVol[ classData3d==classWM ] <- 1
  ...
  if (parameterSegmentationProp[1,counter]!=0) {
    ...
    wmStep01Volume <- wmStep01$brk[,,,classWM]
    ...
    for (kSplit in 1:(length( limitsSize )-1) ) {
      ...
      outVol[ emptyVol > parameterSegmentationProp[1,counter]  ] <- 1
    }
    
    write.AFNI( filename='wmMask_temp.nii.gz',
                brk = outVol,
                origin = wmStep01$origin, 
                delta = wmStep01$delta, 
                defhead = wmStep01$NI_head )
    system( '3dmask_tool -input wmMask_temp.nii.gz -fill_holes -dilate_input 1 -1 -prefix wmMask.nii.gz' )
  }
  ...
  system( instr )
  out <- tryCatch( clustTable <- read.table( 'out.1D', comment.char = "#" ) )
  system( 'rm out.1D' )
...
for ( counter in c(3,4) ) {
  #for ( counter in 1:2 ) {
  setwd(mainDir)
  if (counter==1) {
    system( sprintf( '@clip_volume -input %s -superior %s -anterior %1.0f -prefix clip_0%1.0f.nii.gz', 'inputAnat_mask_box.nii.gz', limitsIS[2], limits[2], counter ) )
  }
  if (counter>1) {
    system( sprintf( '@clip_volume -input %s -superior %s -posterior %1.0f -anterior %1.0f -prefix clip_0%1.0f.nii.gz', 'inputAnat_mask_box.nii.gz', limitsIS[2], limits[counter], limits[counter+1], counter ) )
  }
  instr <- sprintf('3dcalc -a clip_0%1.0f.nii.gz -expr \u0027step(a)\u0027 -prefix maskVolume.nii.gz', counter)
  system( instr )
  ...
  system( instr )
  system('rm maskVolume.nii.gz')
  system( sprintf('mv Segsy Segsy_0%1.0f_01', counter) )
  ...
  setwd( sprintf('Segsy_0%1.0f_01', counter) )
  ...
  if (parameterSegmentationClass[1,counter]==0) { classWM <- 3 }
  if (parameterSegmentationClass[1,counter]!=0) { classWM <- parameterSegmentationClass[1,counter] }
  ...
  if (parameterSegmentationProp[1,counter]!=0) {
    wmStep01 <- read.AFNI( 'Posterior+orig' )
    wmStep01Volume <- wmStep01$brk[,,,classWM]
    emptyVol <- array(0, dim( classData3d ) )
    selWmStep02 <- wmStep01Volume > parameterSegmentationProp[1,counter]
    emptyVol[ selWmStep02 ] <- 1
    write.AFNI( filename='wmMask_temp.nii.gz',
                brk = emptyVol,
                origin = classVol$origin, 
                delta = classVol$delta, 
...
for ( counter in c(3,4) ) {
  ...
  setwd(mainDir)
  if (counter==1) {
    system( sprintf( '@clip_volume -input %s -inferior %s -superior %s -anterior %1.0f -prefix clip_0%1.0f.nii.gz', 'inputAnat_mask_box.nii.gz', limitsIS[2], limitsIS[3], limits[2], counter ) )
  }
  if (counter>1) {
    system( sprintf( '@clip_volume -input %s -inferior %s -superior %s -posterior %1.0f -anterior %1.0f -prefix clip_0%1.0f.nii.gz', 'inputAnat_mask_box.nii.gz', limitsIS[2], limitsIS[3], limits[counter], limits[counter+1], counter ) )
  }
  instr <- sprintf('3dcalc -a clip_0%1.0f.nii.gz -expr \u0027step(a)\u0027 -prefix maskVolume.nii.gz', counter)
  system( instr )
  ...
  system( instr )
  system('rm maskVolume.nii.gz')
  system( sprintf('mv Segsy Segsy_0%1.0f_02', counter) )
  ...
  setwd( sprintf('Segsy_0%1.0f_02', counter) )
  ...
  if (parameterSegmentationClass[2,counter]==0) { classWM <- 3 }
  if (parameterSegmentationClass[2,counter]!=0) { classWM <- parameterSegmentationClass[2,counter] }
  
  if (parameterSegmentationProp[2,counter]==0) {
    emptyVol <- array(0, dim( classData3d ) )
    emptyVol[ classData3d==classWM ] <- 1
    classVol$brk[,,,1] <- emptyVol
    write.AFNI( filename='wmMask.nii.gz',
  ...
  if (parameterSegmentationProp[2,counter]!=0) {
    ...
    wmStep01Volume <- wmStep01$brk[,,,classWM]
    emptyVol <- array(0, dim( classData3d ) )
    selWmStep02 <- wmStep01Volume > parameterSegmentationProp[2,counter]
    emptyVol[ selWmStep02 ] <- 1
    write.AFNI( filename='wmMask_temp.nii.gz',
                brk = emptyVol,
                origin = classVol$origin, 
...
for ( counter in c(3,4) ) {
  ...
  setwd(mainDir)
  if (counter==1) {
    system( sprintf( '@clip_volume -input %s -inferior %s  -anterior %1.0f -prefix clip_0%1.0f.nii.gz', 'inputAnat_mask_box.nii.gz', limitsIS[3], limits[2], counter ) )
  }
  if (counter>1) {
    system( sprintf( '@clip_volume -input %s -inferior %s  -posterior %1.0f -anterior %1.0f -prefix clip_0%1.0f.nii.gz', 'inputAnat_mask_box.nii.gz', limitsIS[3], limits[counter], limits[counter+1], counter ) )
  }
  instr <- sprintf('3dcalc -a clip_0%1.0f.nii.gz -expr \u0027step(a)\u0027 -prefix maskVolume.nii.gz', counter)
  system( instr )
  ...
  system( instr )
  system('rm maskVolume.nii.gz')
  system( sprintf('mv Segsy Segsy_0%1.0f_03', counter) )
  ...
  setwd( sprintf('Segsy_0%1.0f_03', counter) )
  ...
  if (parameterSegmentationClass[3,counter]==0) { classWM <- 3 }
  if (parameterSegmentationClass[3,counter]!=0) { classWM <- parameterSegmentationClass[3,counter] }
  
  if (parameterSegmentationProp[3,counter]==0) {
    emptyVol <- array(0, dim( classData3d ) )
    emptyVol[ classData3d==classWM ] <- 1
    classVol$brk[,,,1] <- emptyVol
    write.AFNI( filename='wmMask.nii.gz',
  ...
  if (parameterSegmentationProp[3,counter]!=0) {
    ...
    wmStep01Volume <- wmStep01$brk[,,,classWM]
    emptyVol <- array(0, dim( classData3d ) )
    selWmStep02 <- wmStep01Volume > parameterSegmentationProp[3,counter]
    emptyVol[ selWmStep02 ] <- 1
    write.AFNI( filename='wmMask_temp.nii.gz',
                brk = emptyVol,
                origin = classVol$origin, 
...
for ( counter in c(1,2,5,6) ) {
  ...
  CMvalue <- CMvalues[ 3  ]
  system('rm _ttt_cm.1D')
  instr <- sprintf( '@clip_volume -input clip_0%1.0f.nii.gz -superior %1.0f -prefix clip_0%1.0f_clip.nii.gz', counter, CMvalue, counter )
  system( instr )
  system(sprintf('rm clip_0%1.0f.nii.gz',counter))
  system(sprintf('mv clip_0%1.0f_clip.nii.gz clip_0%1.0f.nii.gz',counter,counter))
...
for (k in 1:length( dirSegmentation ) ) {
  if (k==1) { instrVar <- sprintf( '-%s %s/wmMask_clust_fill_distance_clust_fill_dil_er.nii.gz', letterArray[k], dirSegmentation[k] ) }
  if (k>1) { instrVar <- sprintf( '%s -%s %s/wmMask_clust_fill_distance_clust_fill_dil_er.nii.gz', instrVar, letterArray[k], dirSegmentation[k] ) }
