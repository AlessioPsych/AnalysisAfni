...
rm(list=ls())
setwd('/data1/projects/myelin/myelinData/hemiBackup/hemianopticdata/V6512.hemianoptic/SegmentGrayMatter_Test_JD')
...
setwd('/data1/projects/myelin/myelinData/hemiBackup/hemianopticdata/V6512.hemianoptic/SegmentGrayMatter_test_noCB')
args <- c('pdCorrectRegularT1_noBlur_stripped.nii',
          '/packages/afni/17.0.13','/data1/projects/myelin/analysisAfni/surfaces','/packages/afni/17.0.13',
          '6.5-6.5-7-7-6.5-6.5','10','white_matter_mask_0.01_0.09_20_5-5-5-8-8-8.nii.gz')

setwd('/home/dijkj/Documents/Segmentation_tests/JeDi')
args <- c('anatomy_res.nii.gz',
          '/packages/afni/17.0.13','/data1/projects/myelin/analysisAfni/surfaces','/packages/afni/17.0.13',
          '6.5-6.5-7-7-6.5-6.5','10','white_matter_mask_02_08_30_8-8-8-8-8-8_bestSoFar.nii.gz')
setwd('/home/dijkj/Documents/Segmentation_tests/JeDi/Segment_test_noCB')
args <- c('anatomy_res.nii.gz',
          '/packages/afni/17.0.13','/data1/projects/myelin/analysisAfni/surfaces','/packages/afni/17.0.13',
          '6.5-6.5-7-7-6.5-6.5','10','white_matter_mask_02_08_30_8-8-8-8-8-8.nii.gz')
...
# Get rid of this bit, and instead call removeCerebellum.sh
instr <- sprintf('removeCerebellum.sh %s %s 1', args[1], args[4])
system(instr)
...
# system( instr )
# 
# ### mask input to exclude cerebellum
...
#replace upt till here

# find the centers, the space and set the limits accordingly
instr <- '3dAutobox -noclust -prefix inputAnat_mask_box.nii.gz -input anatomy_noCB.nii.gz'
system( instr )
...
for ( counter in 1:(length(limits)-1) ) {
  ...
  # system( instr )
  # instr <- '3dkmeans -f volumeN3.nii.gz -mask maskN3_01.nii.gz -k 4 -prefix kMeansOut04.nii.gz -r 200'
  # system( instr )  
  
  instr <- '3dcalc -a volumeN3.nii.gz -b white_matter_automask.nii.gz -expr \u0027 a*within(b,0,0) \u0027 -prefix volumeN3_mask.nii.gz'
  system( instr ) #Volume with only the white matter
  instr <- '3dcalc -a volumeN3_mask.nii.gz -expr \u0027step(a)\u0027 -prefix maskN3.nii.gz'
  system( instr ) #Volume with everything but the white matter, zero around the brain as well.
  instr <- '3dcalc -a volumeN3.nii.gz -expr \u0027step(a)\u0027 -prefix maskN3_01.nii.gz'
  system( instr ) #same as previous line, but with the white matter included in the mask
  
  #idea: combine wm_definition in volumeN3 so that wm is uniform, then run kmeans. use sth like ln 142
  instr <- '3dcalc -a volumeN3.nii.gz -b white_matter_automask.nii.gz -expr \u0027 a*within(b,1,1) \u0027 -prefix wm.nii.gz'
  system( instr ) # Get just the white matter of the current volume
  instr <- '3dBrickStat -non-zero -mean wm.nii.gz'
  m <- as.character( as.numeric( system( instr, intern = TRUE ))) #Get the mean value of the white matter, store as m
  instr <- sprintf('3dcalc -a white_matter_automask.nii.gz -b wm.nii.gz -c maskVolume.nii.gz -expr \u0027 a*%s*c \u0027 -prefix wm_mask_scaled.nii.gz', m)
  system( instr ) # Scale the wm mask and restrict to current slab
  # instr <- sprintf('3dcalc -a wm_mask_scaled.nii.gz -b maskVolume.nii.gz -expr \u0027 a*b \u0027 -prefix wm_mask_scaled_clip.nii.gz', m)
  # system( instr ) # Restrict the wm mask to the current slab
  
  instr <- '3dcalc -a volumeN3.nii.gz -b maskN3.nii.gz -expr \u0027 a*step(b) \u0027 -prefix volumeN3_noWM.nii.gz'
  system( instr )
  instr <- '3dcalc -a volumeN3_noWM.nii.gz -b wm_mask_scaled.nii.gz -expr a+b -prefix volumeN3_scaledWM.nii.gz'
  system( instr ) #Volume with everything, with the wm scaled to the max of the wm value in the slab 
  
  
  # #kmeans try
  # instr <- '3dkmeans -f volumeN3_scaledWM.nii.gz -mask maskN3_01.nii.gz -k 3 -prefix kMeansOut01.nii.gz -r 200'
  # system( instr )  #Best for Segmentation_tests/JeDi. Most accurate GM for mp2rage
  # instr <- '3dkmeans -f volumeN3_scaledWM.nii.gz -mask maskN3_01.nii.gz -k 4 -prefix kMeansOut02.nii.gz -r 200'
  # system( instr )  
  # 
  # instr <- '3dkmeans -f volumeN3.nii.gz -mask maskN3.nii.gz -k 3 -prefix kMeansOut03.nii.gz -r 200'
  # system( instr ) 
  # instr <- '3dkmeans -f volumeN3.nii.gz -mask maskN3.nii.gz -k 2 -prefix kMeansOut04.nii.gz -r 200'
  # system( instr ) 
  
  # #let's try a unifize step
  # instr <- '3dUnifize -GM -prefix volumeN3_scaledWM_unif.nii.gz -input volumeN3_scaledWM.nii.gz'
  # system( instr ) 
  # instr <- '3dkmeans -f volumeN3_scaledWM_unif.nii.gz -mask maskN3_01.nii.gz -k 3 -prefix kMeansOut05.nii.gz -r 200'
  # system( instr )
  # instr <- '3dkmeans -f volumeN3_scaledWM_unif.nii.gz -mask maskN3_01.nii.gz -k 4 -prefix kMeansOut07.nii.gz -r 200'
  # system( instr )
  # # instr <- '3dkmeans -f volumeN3_scaledWM_unif.nii.gz -mask maskN3_01.nii.gz -k 6 -prefix kMeansOut08.nii.gz -r 200'
  # # system( instr )
  # # instr <- '3dkmeans -f volumeN3_scaledWM_unif.nii.gz -mask maskN3_01.nii.gz -k 2 -prefix kMeansOut09.nii.gz -r 200'
  # # system( instr )
  # 
  # # fewer iterations
  # instr <- '3dkmeans -f volumeN3.nii.gz -mask maskN3.nii.gz -k 2 -prefix kMeansOut04a.nii.gz -r 50'
  # system( instr ) 
  # instr <- '3dkmeans -f volumeN3.nii.gz -mask maskN3.nii.gz -k 2 -prefix kMeansOut04b.nii.gz -r 10'
  # system( instr ) 
  # 
  
  # #3dSeg tries
  # 
  # instr <- sprintf('3dSeg -anat volumeN3_scaledWM.nii.gz -mask maskVolume.nii.gz -classes \u0027CSF; GM; WM\u0027 -bias_classes \u0027GM; WM\u0027 -bias_fwhm 0.0 -mixfrac UNI -main_N %s -blur_meth BFT', args[6] ) 
  # print( instr )
  # system( instr )
  # 
  # #Force 3dSeg on volume without WM
  # instr <- sprintf('3dSeg -anat volumeN3.nii.gz -mask maskN3.nii.gz -classes \u0027CSF; GM\u0027 -bias_classes \u0027GM\u0027 -bias_fwhm 0.0 -mixfrac UNI -main_N %s -blur_meth BFT', args[6] ) 
  # print( instr )
  # system( instr )
  
  # more iterations
  instr <- sprintf('3dSeg -anat volumeN3.nii.gz -mask maskN3.nii.gz -classes \u0027CSF; GM\u0027 -bias_classes \u0027GM\u0027 -bias_fwhm 0.0 -mixfrac UNI -main_N 1000 -blur_meth BFT') 
  print( instr )
  system( instr )
  
  # instr <- sprintf('conformToTlrc.sh %s %s/TT_N27+tlrc 1', args[1], atlasDir )
  # system( instr ) # why? 
