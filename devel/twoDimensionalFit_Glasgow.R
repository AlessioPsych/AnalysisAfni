...
setwd('/analyse/Project0226/GN18NE278_KMA25_FEF_28092018_nifti')
mainDir <- getwd()
args <- c('bars_mask_res.nii.gz', 'barsTs_res.nii.gz', 'output_test_newStim','/home/alessiof/abin', '1', '4','1')
...
setwd('/analyse/Project0226/scanner_train - Copy/prfbars_nomrstim_large_field_glas_get_stimuli')
arrayStim <- scan( 'prfStim.txt' )
setwd(mainDir)
#stimMat <- aperm( array( arrayStim, c(128,620,96) ), c( 1, 3, 2 ) )
stimMat <- aperm( array( arrayStim, c(128,620,96) ), c( 3, 1, 2 ) )
...
for ( snap in 1:dim(stimMat)[3] ) {
  image( stimMat[,,snap], axes=FALSE ); par(new=TRUE); Sys.sleep(0.02) 
}
stimSeq <- stimMatFlip
...
print('get hrf...')
hrf <- canonicalHRF( seq(0,30,0.5) )

addSpace <- abs( min(x) )*0.5
print('build prediction...')
xPosFit <- seq( min(x)-addSpace, max(x)+addSpace, length.out=10 )
yPosFit <- seq( min(y)-addSpace, max(y)+addSpace, length.out=15 )
sigmaArrayPositive <- seq( 0.15, 7, length.out=10 )
