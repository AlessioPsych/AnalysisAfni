...
for ( participantsFoldersIndex in seq( 2,length( participantsFolders )-1 ) ) { #seq( 1,length( participantsFolders )
  ...
  instr <- paste( '3dSurf2Vol',
                  ...
                  '-map_func max',
                  '-f_steps 60',
                  '-sdata std.141.rh.economo.annot.niml.dset',
                  '-prefix vonEconomoAtlasRH.nii.gz')
  system( instr )
  
  
  # roi
  instr <- sprintf( '3dcalc -a vonEconomoAtlasRH.nii.gz -expr \u027 within(a,17.5,19.5) \u027  -prefix von_economo_roi_out_or_RH.nii.gz'  ); system( instr )
  instr <- sprintf('3dresample -master %s -rmode NN -inset von_economo_roi_out_or_RH.nii.gz -prefix von_economo_roi_out_RH.nii.gz', filesReference[1] ); system( instr )
  ...
  roiFile <- read.AFNI('von_economo_roi_out_RH.nii.gz')
