...
paramsFiles <- list.files( path=getwd(), pattern='_R2_*', recursive = FALSE, full.names = FALSE  )

allParamsTemp <- c( paramsFiles )
allParams <- c()
for ( n in 1:length(allParamsTemp) ) {
  tempStr <- strsplit(allParamsTemp[n],'ASM16')[[1]][2]
...
setwd( resultsFolder )
print( getwd() )
dirToCheck <- sprintf('KastnerClassic_contrastBased' )
...
for ( nModels in 1:length( allParams ) ) { # nModels <- 1
  outModelDf <- c()
  for ( nPart in 1:length( modelParticipants )  ) { # nPart <- 3 # length( modelParticipants )
    ...
    storedNames <- names( outModelDfTemp )
    newNames <- c( storedNames[1:4], 'R2', 'intercept' , 'slope', storedNames[8:10] ) 
    names( outModelDfTemp ) <- newNames
    outModelDf <- rbind( outModelDf, outModelDfTemp )
    
  }
  save( outModelDf, file = sprintf( '%s/%s/%s.RData', resultsFolder, 'KastnerClassic_contrastBased', allParams[ nModels ] ) )
}
