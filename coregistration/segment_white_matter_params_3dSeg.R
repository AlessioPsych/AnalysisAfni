...
#rm(list=ls())
#setwd('/media/alessiof/Data/tests/testInHouseSegmentation')
#args <- c('testAnat.nii.gz','/home/alessiof/abin','/home/alessiof/Programs/dlDevelopment/analysisAfni/surfaces','/home/alessiof/abin','0.001','0.07','5','6-6-6-6-6-6','/home/alessiof/Programs/dlDevelopment/analysisAfni/coregistration','0','0','1')
...
# save the posterior probability
setwd(mainDir)
dirSegmentation <- dir( pattern=sprintf( 'Segsy*') )
letterArray <- letters[1:length( dirSegmentation )]
for (k in 1:length( dirSegmentation ) ) {
  if (k==1) { instrVar <- sprintf( '-%s %s/Posterior+orig[%s]', letterArray[k], dirSegmentation[k], as.character(0) ) }
  if (k>1) { instrVar <- sprintf( '%s -%s %s/Posterior+orig[%s]', instrVar, letterArray[k], dirSegmentation[k], as.character(0) ) }
  if (k==1) { instrExpr <- sprintf( '\u0027%s', letterArray[k] ) }
  if (k>1) { instrExpr <- sprintf( '%s + %s',instrExpr, letterArray[k] ) } 
  if (k==length( dirSegmentation )) { instrExpr <- sprintf( '%s\u0027', instrExpr, letterArray[k] ) } 
}
instrCalc <- sprintf('3dcalc %s -expr %s -prefix posteriorProb_csf.nii.gz', instrVar, instrExpr)
system( instrCalc )
...
letterArray <- letters[1:length( dirSegmentation )]
for (k in 1:length( dirSegmentation ) ) {
  if (k==1) { instrVar <- sprintf( '-%s %s/Posterior+orig[%s]', letterArray[k], dirSegmentation[k], as.character(1) ) }
  if (k>1) { instrVar <- sprintf( '%s -%s %s/Posterior+orig[%s]', instrVar, letterArray[k], dirSegmentation[k], as.character(1) ) }
  if (k==1) { instrExpr <- sprintf( '\u0027%s', letterArray[k] ) }
  if (k>1) { instrExpr <- sprintf( '%s + %s',instrExpr, letterArray[k] ) } 
  if (k==length( dirSegmentation )) { instrExpr <- sprintf( '%s\u0027', instrExpr, letterArray[k] ) } 
}
instrCalc <- sprintf('3dcalc %s -expr %s -prefix posteriorProb_gm.nii.gz', instrVar, instrExpr)
system( instrCalc )
...
letterArray <- letters[1:length( dirSegmentation )]
for (k in 1:length( dirSegmentation ) ) {
  if (k==1) { instrVar <- sprintf( '-%s %s/Posterior+orig[%s]', letterArray[k], dirSegmentation[k], as.character(2) ) }
  if (k>1) { instrVar <- sprintf( '%s -%s %s/Posterior+orig[%s]', instrVar, letterArray[k], dirSegmentation[k], as.character(2) ) }
  if (k==1) { instrExpr <- sprintf( '\u0027%s', letterArray[k] ) }
  if (k>1) { instrExpr <- sprintf( '%s + %s',instrExpr, letterArray[k] ) } 
  if (k==length( dirSegmentation )) { instrExpr <- sprintf( '%s\u0027', instrExpr, letterArray[k] ) } 
}
instrCalc <- sprintf('3dcalc %s -expr %s -prefix posteriorProb_wm.nii.gz', instrVar, instrExpr)
system( instrCalc )
