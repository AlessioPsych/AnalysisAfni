...
roiAtlas <- '/analyse/Project0165/Fracasso_Anatomy/data_ins/dataFreeSurfer/suma_MNI152_2009_princetonAtlas'
saveRoi <- '/analyse/Project0165/Fracasso_Anatomy/AHEAD_database/10007840/results/insula_vonEconomo'
appendFilename <- 'LH'
...
for ( participantsFoldersIndex in seq( 2,length( participantsFolders )-1 ) ) { #seq( 1,length( participantsFolders )
  #participantsFoldersIndex <- 4
  setwd( mainDir )
  setwd( partDir )
  print( '...' )
  print( '...' )
  ...
  setwd( dir()[1] )
  currentDir = getwd()
  instr <- sprintf( 'cp %s/anatCopy_1mm.nii.gz %s/AHEAD_test/SUMA/', currentDir, currentDir)
  system(instr)
  print( dir() )
  setwd( 'AHEAD_test' )
  setwd( 'SUMA' )
  ...
  #copy atlas
  instr <- sprintf( 'cp %s/*economo.annot.niml* %s', roiAtlas, currentDir )
  system( instr )
  anatIn = "anatCopy_1mm.nii.gz"
  filenameIn <- filesReference
  instr <- paste( '3dSurf2Vol',
                  '-spec std.141.AHEAD_test_lh.spec',
                  '-surf_A std.141.lh.white.gii',
                  '-surf_B std.141.lh.pial.gii',
                  sprintf('-sv %s', anatIn ),
                  sprintf('-grid_parent %s', anatIn ),
                  '-map_func max',
                  '-f_steps 60',
                  '-sdata std.141.lh.economo.annot.niml.dset',
                  '-prefix vonEconomoAtlasLH.nii.gz')
  system( instr )

  

  # roi
  instr <- sprintf( '3dcalc -a vonEconomoAtlasLH.nii.gz -expr \u027 within(a,17.5,19.5) \u027 -prefix von_economo_roi_out_or_LH.nii.gz'  ); system( instr )
  instr <- sprintf('3dresample -master %s -rmode NN -inset von_economo_roi_out_or_LH.nii.gz -prefix von_economo_roi_out_LH.nii.gz', filesReference[1] ); system( instr )
  ...
  roiFile <- read.AFNI('von_economo_roi_out_LH.nii.gz')
  ...
  profileFun <- function( profilesVolume, nameIn, subjectName ) {
    roiIdx <- which( depthVolume>0.15 & depthVolume<0.85 & abs(curvatureVolume) > 0.0000001 & abs(thicknessVolume) > 0.0000001 & roiVolume==1 )
