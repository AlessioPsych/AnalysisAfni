...
testSubjectDir <- '/analyse/Project0226/KastnerModel/staging_area_Kastner/anatomies_KastnerClassic_Freesurfer/ASM16_ANATOMY/FreeSeg_result/SUMA/kastnerClassicModelsSurfaces1D'
...
dirToCheck <- sprintf('%s/kastnerClassicAverageModelsSurfaces1D_hrf_correction', surfaceFolder )
...
for ( nAverages in 1:length( cleanFilenames ) ) { #length( cleanFilenames ) #nAverages <- 1
  ...
  write.table( x=round(fileStore,4), file=sprintf('%s/kastnerClassicAverageModelsSurfaces1D_hrf_correction/%s', surfaceFolder, fileNameOutput ), col.names = FALSE, row.names = FALSE )
