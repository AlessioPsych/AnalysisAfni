for (nSubj in 7:8) {
  ...
  #### glm on concatenated ts approach, need to apply distortion correction a posteriori on stat map ####
  if ( dir.exists('test_concatenated.results/') ) { system('rm -R test_concatenated.results/') }
  if ( file.exists('proc.test_concatenated') ) { system('rm proc.test_concatenated') }
  if ( file.exists('output.proc.test_concatenated') ) { system('rm output.proc.test_concatenated') }
  rString01 <- paste( replicate(4, "\u0027BLOCK(25,1)\u0027"), collapse = " " )
  rString02 <- "\u0027BLOCK(5,1)\u0027"
  
  instr <- paste('afni_proc.py',
                 '-subj_id test_concatenated',
                 '-dsets EPI_timeSlice/*.nii',
                 ...
                 sprintf('-gltsym \u0027SYM: +0.5*active +0.5*passive -1.0*touch\u0027'),
                 sprintf('-glt_label 5 active_hf+passive_hf_vs_touch'),
                 sprintf('-gltsym \u0027SYM: +active +passive -touch -instructions\u0027'),
                 sprintf('-glt_label 6 active+passive-touch-instructions'),
                 sprintf('-gltsym \u0027SYM: +active -passive\u0027'),
                 sprintf('-glt_label 7 active-passive'),
                 sprintf('-gltsym \u0027SYM: +active -rest -passive -rest\u0027'),
                 sprintf('-glt_label 8 active_rest-passive_rest'),
