...
for ( nEpi in 1:length(epiFiles) ) {
  ...
  instr <- sprintf('3dvolreg -maxite 38 -twopass -verbose -zpad 1 -base %s[%1.0f] -1Dfile %s -1Dmatrix_save %s -prefix %s -heptic %s ', epiFiles[selectedEpi], nTRs-1, motion1DfileLin, motion1DfileAff, prefixName, epiFiles[nEpi] )
  print( instr )
  system( instr )
}
