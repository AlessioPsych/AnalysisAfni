...
dirToCheck <- sprintf('%s/kastnerClassic_test', modelsDir )
print( dirToCheck )
...
for ( nPart in 1:length(cwParticipants) ) { #length(cwParticipants) # nPart <- 1
  ...
  #### create outputFolder folder, kastnerClassic ####
  dirToCheck <- sprintf('%s/kastnerClassic_test/%s', modelsDir, participantName )
  print( dirToCheck )
  if ( dir.exists( dirToCheck ) ) {
    instr <- sprintf( 'rm -R %s', dirToCheck ) 
    print( instr )
  ...
  instr <- paste('3dresample',
                 ...
                 sprintf( '-prefix %s/%s/%s/%s.zzz.lh.ribbon.nii', modelsDir, 'kastnerClassic_test', participantName, selectedAnatFolders[nPart] ), 
                 '-rmode NN' )
  print( instr )
  if (runCodeFlag==1) { system( instr ) }  
  ...
  instr <- paste('3dresample',
                 ...
                 sprintf( '-master %s', epiCwTemp ),  
                 sprintf( '-prefix %s/%s/%s/%s.zzz.rh.ribbon.nii', modelsDir, 'kastnerClassic_test', participantName, selectedAnatFolders[nPart] ), 
                 '-rmode NN' )
  print( instr )
  if (runCodeFlag==1) { system( instr ) }  
  ...
  # combine ribbons
  instr <- paste('3dcalc', 
                 sprintf( '-a %s/%s/%s/%s.zzz.lh.ribbon.nii', modelsDir, 'kastnerClassic_test', participantName, selectedAnatFolders[nPart] ), 
                 sprintf( '-b %s/%s/%s/%s.zzz.rh.ribbon.nii', modelsDir, 'kastnerClassic_test', participantName, selectedAnatFolders[nPart] ), 
                 sprintf('-expr \u0027a+b\u0027'),
                 sprintf( '-prefix %s/%s/%s/%s.zzz.grayMatter.nii', modelsDir, 'kastnerClassic_test', participantName, selectedAnatFolders[nPart] ) ) 
  print( instr )
  if (runCodeFlag==1) { system( instr ) } 
  
  # get mask
  instr <- sprintf('3dmask_tool -input %s_ANATOMY.zzz.grayMatter.nii -prefix %s_mask_auto.nii.gz -fill_holes', participantName, participantName )  
  print( instr )
  if (runCodeFlag==1) { system( instr ) }
  ...
  instr <- paste('cp',
                 sprintf( '%s', epiCwTemp ),  
                 sprintf( '%s/%s/%s/%s', modelsDir, 'kastnerClassic_test', participantName, cwParticipants[nPart] )
  )
  print( instr )
  if (runCodeFlag==1) { system( instr ) } 
  ...
  instr <- paste('cp',
                 ...
                 sprintf( '%s/%s/%s/%s', modelsDir, 'kastnerClassic_test', participantName, ccwParticipants[nPart] )
  )
  print( instr )
  if (runCodeFlag==1) { system( instr ) } 
