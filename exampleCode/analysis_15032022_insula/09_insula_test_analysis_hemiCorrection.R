...
for (nFile in 1:length(filesList) ) { #nFile <- 146; length(filesList) length(filesList)
  ...
  allParticipantsData[[length(allParticipantsData) + 1]] <- aggregateProfilesMat_01
  dfSubj1 <- cbind( aggregateCoords_01 )
  dfSubjAll = rbind(dfSubjAll,dfSubj1)
  ...
  rm( anatFile, 
      ...
      keepMacroVoxel,
      modelMacroVoxels_01,
      aggregateProfiles_01, 
      aggregateProfilesMat_01, 
      aggregateCoords_01 ); gc();
...
#save(aggregateProfilesMat_combined, file = "/analyse/Project0165/Fracasso_Anatomy/AHEAD_database/10007840/analysis_15032022/aggregateProfilesMat_combined_AHEAD.RData")
#load(aggregateProfilesMat_combined, file = "/analyse/Project0165/Fracasso_Anatomy/AHEAD_database/10007840/analysis_15032022/aggregateProfilesMat_combined_AHEAD.RData")
...
save.image("/analyse/Project0165/Fracasso_Anatomy/AHEAD_database/10007840/analysis_15032022/09_data_lm_save.RData")


#load("/analyse/Project0165/Fracasso_Anatomy/AHEAD_database/10007840/analysis_15032022/09_data_save.RData")
#load("/analyse/Project0165/Fracasso_Anatomy/AHEAD_database/10007840/analysis_15032022/09_Age1_data_save.RData")
#load("/analyse/Project0165/Fracasso_Anatomy/AHEAD_database/10007840/analysis_15032022/09_Age2_data_save.RData")
...
plot_silhouette = function(LH_SH, RH_SH, dataframe_name){
  stats_LH = col_wise_stats(LH_SH)
  stats_RH = col_wise_stats(RH_SH)
  ...
  # Determine highest point for annotation
  highest_point = combined_stats %>% arrange(desc(Mean)) %>% select(Mean,Cluster_no) 
  highest_point = highest_point[1,]

  # Calculate rounded max for y-axis
  y_max = round(max(combined_stats$Mean), 1) + 0.1 

  # Plotting
  my_colors = c("LH" = "red3", "RH" = "green3")
  p = ggplot(combined_stats, aes(x = Cluster_no, y = Mean, color = Hemisphere, group = Hemisphere)) +
    geom_line(size = 1.8) +
    geom_point(size = .5, colour = 'black') +
    geom_errorbar(aes(ymin = Mean - SD, ymax = Mean + SD, color = Hemisphere), width = 0.5, size = 1) +
    theme_minimal() +
    labs(x = "Number of Clusters", y = "Mean Silhouette Score", color = "Hemisphere") +
    scale_x_continuous(breaks = c(1,2, 10), limits = c(1, 10)) +
    scale_y_continuous(limits = c(0, 0.5), breaks = c(0, 0.5)) + # Updated this line for dynamic y-axis
    ...
    annotate("text", x = highest_point$Cluster_no, y = highest_point$Mean, 
             label = "Optimal No of Clusters", vjust = 15, hjust = -0.06, color = "black", size = 5) +
    geom_vline(xintercept = highest_point$Cluster_no, linetype = "dotted", color = "black", size = 1.5) +
    annotate("text", x = mean(range(combined_stats$Cluster_no)), y = y_max * 0.75, label = dataframe_name, size = 6, color = "black")
  return(p)
  # Display the plot
  #x11(width = 5.5, height = 3.5)
  #print(p)
  # Optionally save the plot
  #dev.copy2pdf(file = sprintf('%s/%s%s.pdf', figureDir,"silhouette_" ,dataframe_name), width = 5.5, height = 3.5)
  #dev.off()
}
...
p1 <- plot_silhouette(S1_LH, S1_RH, "Subject 1") + theme(legend.position = "none")
p2 <- plot_silhouette(S2_LH, S2_RH, "Subject 2") + theme(legend.position = "none")
p3 <- plot_silhouette(S3_LH, S3_RH, "Subject 3") + theme(legend.position = "none")
p4 <- plot_silhouette(LH_SH, RH_SH, "Average") # Keep legend on this plot

x11(width = 18, height = 4)
grid.arrange(p1, p2, p3, p4, ncol = 4)
dev.copy2pdf(file = sprintf("%s/AHEAD_silhouette_plot.pdf",figureDir),width = 18, height = 4)

#ggsave(sprintf("%s/AHEAD_silhouette_plot.pdf",figureDir), plot = combined_plot, width = 17.5, height = 2.5, units = "cm") 
...
plotData <- function(data, plotname) {
  ...
  pdfFilename <- paste0(figureDir, plotname, "_high_low_Myelin.pdf")
  dev.copy2pdf(file = pdfFilename)
  ifelse(nFile > 1 ,dev.off,print(""))
}

ifelse(nFile > 2, plotData(aggregateProfilesMat_combined, "AHEAD_Average"),plotData(aggregateProfilesMat_combined, participantName))
#plotData(aggregateProfilesMat_combined, "AHEAD_Age1_Average")
#plotData(aggregateProfilesMat_combined, "AHEAD_Age2_Average")



#### linear model #####

aggregateProfilesMat_combined
