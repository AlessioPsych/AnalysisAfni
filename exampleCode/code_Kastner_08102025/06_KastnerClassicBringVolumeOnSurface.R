...
for ( nPart in 1:length( modelParticipants )  ) { #nPart <- 1# length( modelParticipants )
  ...
  print( getwd() )
  print('#####')
  dirToCheck <- sprintf('kastnerClassicModelsAnatomy_test' )
  ...
  print( getwd() )
  print('#####')
  dirToCheck <- sprintf('kastnerClassicModelsSurfacesNiml_test' )
  if ( dir.exists( dirToCheck ) ) {
    instr <- sprintf( 'rm -R %s', dirToCheck ) 
    print( instr )
    if (runCodeFlag==1) { system( instr ) }  
  ...
  print( getwd() )
  print('#####')
  dirToCheck <- sprintf('kastnerClassicModelsSurfaces1D_test' )
  if ( dir.exists( dirToCheck ) ) {
    instr <- sprintf( 'rm -R %s', dirToCheck ) 
    print( instr )
    if (runCodeFlag==1) { system( instr ) }  
  ...
  for ( nModels in 1:length( allModels ) ) {
    instr <- paste('cp',
                   ...
                   sprintf('%s/%s/FreeSeg_result/SUMA/kastnerClassicModelsAnatomy_test/%s', anatomyDir, selectedAnatFolders[ nPart ], allModels[ nModels ] )
    )
    print( instr )
    if (runCodeFlag==1) { system( instr ) }  
  }
  ...
  for ( nModels in 1:length( allModels ) ) {
    for (outName in c('niml','1D') ) { # outName <- 'niml'
      for (nHemi in c('lh','rh') ) { # nHemi <- 'lh'
        ...
        if (outName=='niml') {
          modelOutputNameInstruction <- sprintf( '-out_niml %s/%s.%s.%s.dset', 'kastnerClassicModelsSurfacesNiml_test', modelOutputName, nHemi, outName ) 
        }
        if (outName=='1D') {
          modelOutputNameInstruction <- sprintf( '-out_1D %s/%s.%s.%s.dset', 'kastnerClassicModelsSurfaces1D_test', modelOutputName, nHemi, outName ) 
        }  
        instr <- paste('3dVol2Surf',
                       ...
                       sprintf( '-sv %s/%s', 'kastnerClassicModelsAnatomy_test', allModels[ nModels ] ),
                       sprintf( '-grid_parent %s/%s', 'kastnerClassicModelsAnatomy_test', allModels[ nModels ] ),
