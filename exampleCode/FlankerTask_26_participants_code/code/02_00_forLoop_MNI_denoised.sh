...
set output_folder = $main_folder/derivatives/processing_afni_MNI_denoised
echo $main_folder
echo $code_folder
echo $input_folder
echo $output_folder
...
foreach i (`cat subjList.txt`)
    echo instr: tcsh $code_folder/02_01_sub_xx_afni_proc_MNI_denoised.sh $i $main_folder $input_folder
    echo instr mv ${i}.results $output_folder
    tcsh $code_folder/02_01_sub_xx_afni_proc_MNI_denoised.sh $i $main_folder $input_folder
