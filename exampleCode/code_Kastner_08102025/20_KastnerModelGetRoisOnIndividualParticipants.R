...
for ( nPart in 1:length( modelParticipants )  ) { # nPart <- 1 # length( modelParticipants )
  ...
  #### create roi folder ####
  dirToCheck <- sprintf('roisWang2015_KastnerModel' )
  ...
  for ( nHemi in c('lh','rh') ) { # nHemi <- 'lh'
    ...
    allVolumeFiles <- list.files( path='KastnerNewModelAnatomy', recursive = FALSE, full.names = FALSE )
    ...
    instr <- paste('3dSurf2Vol',
                   ...
                   sprintf( '-sv %s/%s', 'KastnerNewModelAnatomy', volumeIndividualParticipant ),
                   sprintf( '-grid_parent %s/%s', 'KastnerNewModelAnatomy', volumeIndividualParticipant ),
                   sprintf( '-map_func nzmode' ),
                   sprintf( '-f_steps 15' ),
                   sprintf( '-prefix %s/%s.%s.nii.gz', 'roisWang2015_KastnerModel', 'WangRoi2015', nHemi )
    )
    print( instr )
    if (runCodeFlag==1) { system( instr ) }  
    ...
    instr <- paste('cp',
                   'anatCopy.nii.gz',
                   'roisWang2015_KastnerModel/anatCopy.nii.gz')
    print( instr )
    if (runCodeFlag==1) { system( instr ) }  
