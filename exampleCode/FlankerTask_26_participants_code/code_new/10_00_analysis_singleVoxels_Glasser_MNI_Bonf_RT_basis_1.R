...
inputFiles <- dir( patter='*_RT_correct_basis_1.RData' )
load( inputFiles[1] )
...
dataSelected <- subset( dataSelected_agg, side==sideSelected )
atlasSelected <- lhAtlas
outputFileLh <- 'output_Glasser_lh_ORIG_Denoised_RT_correct_basis_1.nii.gz'
fdrThreshold <- 0.05
...
outputFileRh <- 'output_Glasser_rh_ORIG_Denoised_RT_correct_basis_1.nii.gz'
fdrThreshold <- 0.05
...
if ( file.exists( sprintf('output_Glasser_ORIG_Denoised_combined_RT_correct_basis_1.nii.gz',surfacesFolder) ) ) {
  instr <- sprintf( 'rm %s', sprintf('output_Glasser_ORIG_Denoised_combined_RT_correct_basis_1.nii.gz',surfacesFolder) )
  print( instr )
  system( instr )
}

setwd( surfacesFolder )
instr <- sprintf( '3dcalc -a %s -b %s -expr \u0027a+b\u0027 -prefix output_Glasser_ORIG_Denoised_combined_RT_correct_basis_1.nii.gz', outputFileLh, outputFileRh )
print( instr )
system( instr )
