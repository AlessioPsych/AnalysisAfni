...
x <- seq(-10,10,length.out = dim(stimSeq_eyeMoving)[1] )
y <- seq(-5.5,5.5,length.out = dim(stimSeq_eyeMoving)[2] )
...
hrfDelayOnsetElements <- 2 #1
hrfDelayUnderShootElements <- 2 #1
hrfcParElements <- 2
#multParElements <- 2 #2
...
par_hrf_c <- seq( 0.25, 0.85, length.out=hrfcParElements )
...
predictionGridGlobal <- expand.grid( par_hrf_a1, par_hrf_a2, par_hrf_c, centerVon, kVon, xPosFit_gain, yPosFit_gain, sigmaArrayPositive_gain )
# if (flagSurround==1) {
...
for (modelFitCounter in 1:length(runIndexPredictions) ) { #length(runIndexPredictions)
  ...
  eyeDirectionPrediction <- function(indexPrediction, inputPredictionGrid) {
    ...
    hrf <- canonicalHRF( seq(0,30,samplingTime), param=list(a1=prfPar[1], a2=prfPar[2], b1=0.9, b2=0.9, c=prfPar[3]), verbose=FALSE )
    #plot( canonicalHRF( seq(0,30,samplingTime), param=list(a1=10, a2=15, b1=0.9, b2=0.9, c=0.85), verbose=FALSE )~seq(0,30,samplingTime) )
  ...
  #### visual stimuli predictions ####
  visualStimuliPrediction <- function(indexPrediction, inputPredictionGrid) {
    ...
    directionArray_expanded_radiants <- rep( directionArray, sampleRepetitions ) 
    baselines_expanded <- rep( baselines, sampleRepetitions )
    baselines_expanded <- rep(0,length(baselines_expanded))
    baselines_expanded[ saccStart ] <- 1 #code saccades with 0, baselines with 1, every 200 ms (stimuliSequenceTr)
    
    convTs <- conv( baselines_expanded, hrf )
    convTsTrim <- convTs[1:length(baselines_expanded)]
    timeArray <- seq( 0, length(convTsTrim)*stimuliSequenceTr/1000, length.out = length(convTsTrim) )#  *stimuliSequenceTr/1000
    iTs <- interp1( timeArray, convTsTrim, seq(0,max(timeArray),length.out = dim(ts$brk)[4] ) )
    scaleConvTsTrim <- scaleData( round( convTsTrim, 5 ), 1, 0 )
    scaleIts <- scaleData( round( iTs, 5 ), 1, 0 )
    ...
    # #to plot
    #par(mfrow=c(1,2))
    #plot( baselines_expanded, type='l' )
    #plot( scaleConvTsTrim~timeArray )
    #lines( seq(0,max(timeArray),length.out = dim(ts$brk)[4] ), scaleIts, col='red' )
    
  }
  visualStimuli_Predictions <- sapply( 1:dim(predictionGrid)[1], visualStimuliPrediction, inputPredictionGrid=predictionGrid )
  ...
  timeStimuli <- seq( 0, dim(stimSeq_eyeMoving)[3]*samplingTime, length.out = dim(stimSeq_eyeMoving)[3] )
  mriTime <- seq( 0, dim(stimSeq_eyeMoving)[3]*samplingTime, length.out = dim(ts$brk)[4] )
  ...
  generatePrediction_moving <- function( indexPrediction, inputPredictionGrid ) {
    ...
    a <- dnorm( x, prfPar[6], prfPar[8] )
    b <- dnorm( y, prfPar[7], prfPar[8] )
  ...
  tsPrediction_eyeFix <- t( visualStimuli_Predictions )
  ...
  if (modelFitCounter>1) { 
    selectedCols <- outModel[9,] > outModelLoop[9,] #where r2 is store (after 8 model parameters, there are: r2, beta intercept, beta slope visual, beta slope movement, beta slope direction 
...
fittedPredictionGrid <- t( outModelLoop[seq(1:8),] )
singlePredictorStats <- function( idxLine, inputMat ) {
  ...
  if ( sum(inputMat[idxLine,])!=0 ) {
    ...
    eyeCenterPrediction <- visualStimuliPrediction(idxLine,inputMat)
...
storeAllPred <- cbind( t(outModelLoop[ seq(1,13), ]), t( outModelStats ) )
...
labels <- c('hrf_a1','hrf_a2','hrf_c','centerVonMis','kVonMis','xGain','yGain','sigmaGain',
            'varExp','intercept','b_eyeFix','b_eyeMoving','b_direction','varExp_cont','t_intercept','t_eyeFix','t_eyeMoving','t_direction',
            'p_intercept','p_eyeFix','p_eyeMoving','p_direction')
