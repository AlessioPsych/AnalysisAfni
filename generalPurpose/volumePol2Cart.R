...
print( sprintf( 'rho idx (in afni): %s', rhoIdx ) )
print( sprintf( 'output volume: %s', labelOut ) )

volumePol2Cart <- function( inputVol, thetaIdx, rhoIdx, labelOut  ) {
  instr <- sprintf( '3dcalc -a %s[%s] -b %s[%s] -expr \u0027 b*sin(a) \u0027 -prefix del_x.nii.gz', inputVol, thetaIdx, inputVol, rhoIdx ); system( instr )
  instr <- sprintf( '3dcalc -a %s[%s] -b %s[%s] -expr \u0027 b*cos(a) \u0027 -prefix del_y.nii.gz', inputVol, thetaIdx, inputVol, rhoIdx ); system( instr )
...
volumePol2Cart( inputVol, thetaIdx, rhoIdx, labelOut  )
