...
inputFiles <- dir( patter='*_RT_correct_basis_no.RData' )
load( inputFiles[1] )
...
generateFdrVolume <- function( sideSelected, dataSelected, atlasSelected, outputFile, fdrThreshold, surfacesFolder ) {
  ...
  for ( i in 1:length( rois ) ) { # i <- 1
   print( rois[i] )
    dataSelectedLoop01 <- subset( dataSelected, roi==rois[i] & coefficient=='RT_correct#0_Coef' )
...
dataSelected <- subset( dataSelected_agg, side==sideSelected )
atlasSelected <- lhAtlas
outputFileLh <- 'output_Glasser_lh_ORIG_Denoised_RT_correct_basis_no.nii.gz'
fdrThreshold <- 0.05
...
outputFileRh <- 'output_Glasser_rh_ORIG_Denoised_RT_correct_basis_no.nii.gz'
fdrThreshold <- 0.05
...
if ( file.exists( sprintf('output_Glasser_ORIG_Denoised_combined_RT_correct_basis_no.nii.gz',surfacesFolder) ) ) {
  instr <- sprintf( 'rm %s', sprintf('output_Glasser_ORIG_Denoised_combined_RT_correct_basis_no.nii.gz',surfacesFolder) )
  print( instr )
  system( instr )
}

setwd( surfacesFolder )
instr <- sprintf( '3dcalc -a %s -b %s -expr \u0027a+b\u0027 -prefix output_Glasser_ORIG_Denoised_combined_RT_correct_basis_no.nii.gz', outputFileLh, outputFileRh )
print( instr )
system( instr )
