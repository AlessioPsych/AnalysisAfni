...
if (debugFlag==1) {
  ...
  outputFileName <- 'results_Wang_ORIG_Denoised_RT_IM.RData'
...
getResultsGlasser <- function( mainFolder, inputFolder, outputFolder, outputFileName, roiFilenameLH_input, roiFilenameRH_input, MNI_flag ) {
  ...
  for ( i in  1:length( partDirs ) ) { # i <- 1; side <- 'lh' length( partDirs )
    for ( side in c('lh','rh') ) {
      ...
      print( tsnrFilenameLoop ) 
      print( retinoFilenameLoop ) 
      print( roiFilenameLoop ) 
      
      betaFile <- read.AFNI( statFilenameLoop )
      #statNames <-  strsplit( statFile_names$NI_head$BRICK_LABS$dat, '~' )[[1]]
      statNames <- rep('a', dim(betaFile$brk)[4] )
      coefNames <- sprintf( 'coef%1.0f', 1:48 )
      tNames <- sprintf( 'tStat%1.0f', 1:48 )

      statNames[1] <- c( 'Full_Fstat' )
      statNames[ length(statNames) ] <- c( 'stim01_Fstat' )
      statNames[ seq(2,length(statNames)-1,2) ] <- coefNames		  
      statNames[ seq(3,length(statNames)-1,2) ] <- tNames		  
...
inputFolder <- 'derivatives/processing_afni_denoised_RT_IM'
outputFolder <- 'derivatives/resultsWang'
outputFileName <- 'results_Wang_ORIG_Denoised_singleVoxel_RT_IM.RData'
roiFilenameLH_input <- 'lh.Wang_2015_epiResampled_' 
roiFilenameRH_input <- 'rh.Wang_2015_epiResampled_' 
MNI_flag <- 0
