...
distanceMapFilename = 'DEPTH_al_epi.nii.gz';
...
filterIndex = storePrfMatrix(:,9) >= 0 & storePrfMatrix(:,9) <= 1 & ...
    storePrfMatrix(:,1) > quantile( storePrfMatrix(:,1), 0.35 ) & ...
    storePrfMatrix(:,2) > 0.1 & storePrfMatrix(:,2) < 5 & ...
    storePrfMatrix(:,5) < 5 & storePrfMatrix(:,5) > 0.1 & ...    
    storePrfMatrix(:,7) > 0 & storePrfMatrix(:,7) < 40 & ...
    storePrfMatrix(:,8) > 0 & storePrfMatrix(:,8) < 40;
...
figure(19)
distanceCut = quantile( storePrfMatrixFilter(:,9), linspace(0,1,distanceBinning+1) );
for k = 1 : ( length( distanceCut ) - 1 )
        ...
        eccentricityCut = quantile( eccentricityTemp, linspace(0,1,eccBinning+1) );
...
for k = 1 : ( length( distanceCut ) - 1 )
    ...
    for n = 1:nBootRep
        ...
        meanIntercept(k,n) = median( storePrfMatrixFilterLoop(filterDistance,6) );
...
meanDistance = median( meanDistance, 2 );
