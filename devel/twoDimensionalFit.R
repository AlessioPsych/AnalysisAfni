...
#setwd('/home/fracasso/data/pRF/sub3')
#args <- c('epi_mask.nii.gz','meanTs_short_det.nii.gz','outTest_delme.nii.gz','/packages/afni/17.0.13','1')

#setwd('/media/alessiof/Data/tests/prf_test_intercept')
#args <- c('mask.nii.gz', 'meanTs_test_res.nii.gz', 'output_surround','/home/alessiof/abin', '0', '4','1')
...
#setwd('/home/alessiof/Desktop/scanner_train/prfbars_nomrstim_large_field_glas')
arrayStim <- scan( 'prfStim.txt' )
stimMat <- array( arrayStim, c(384,129,512) )
stimMat <- array( arrayStim, c(129,512,384) )
stimMat <- aperm( array( arrayStim, c(128,620,96) ), c( 1, 3, 2 ) )
stimMatFlip <- stimMat[ ,dim(stimMat)[2]:1, ]
image( stimMatFlip[,,56] )
x11( width=4, height=4 )
for ( snap in 1:dim(stimMat)[3] ) {
  image( stimMatFlip[,,snap] ); par(new=TRUE); Sys.sleep(0.02) 
}


print('get hrf...')
hrf <- canonicalHRF( seq(0,30,trValue[1,1]), list(a1=6,a2=12,b1=0.9,b2=0.9,c=0.35) )

addSpace <- abs( min(x) )*0.5
print('build prediction...')
xPosFit <- seq( min(x)-addSpace, max(x)+addSpace, length.out=9 )
yPosFit <- xPosFit
sigmaArrayPositive <- seq( 0.15, 7, length.out=9 )
...
stimSeqMat <- array( stimSeq, c( length(x)^2, dim(stimSeq)[3] ) )
