dataAxis <- read.table('test06_axis.dset')
out <- cbind( dataAxis[,c(2,3)], rep( 0, dim(dataAxis)[1] ) )
write.table( out, 'flatCoords.1D', row.names = FALSE, col.names = FALSE )
...
# to debug
setwd('/home/fracasso/data/ArjanProject/2017-01-27_HeadcoilSurf_R3_fMRI_AF/ParRec/results/nefertiti')
BOUNDARYNAME='boundary03'
DISTANCEROI='test06.1D.roi'
NODEFACTR=1
KERNELRADIUS=1.25
...
coordsOrig <- read.table('nefertiti.coords.1D')
roi_nodes <-  seq( 0, dim(coordsOrig)[1]-1 )
...
instr <- sprintf( '%s/SurfDist -i_1D nefertiti.coords.1D nefertiti_3.topo.1D -input datasetStart.1D > exampleStart.1D', args[5], args[1], args[1] )
system.time( system( instr ) )
...
write.table( outTemp[,c(2,3)], row.name=FALSE, col.names=FALSE , 'nef_f_coords.1D' )
write.table( outTemp[,c(2,3)], row.name=FALSE, col.names=FALSE , 'nef_f_coords.1D' )
...
setwd('nefertiti')
a <- read.table('nefertiti.topo.1D')
write.table( a[,c(2:4)], 'nefertiti_3.topo.1D', row.names = FALSE, col.names = FALSE )
...
whichUniqueNodes <- sapply( seq( 1, length( sel_roi_nodes) ), nodeIdx, startingNodes=sel_roi_nodes, finalNodes=uniqueNodes )
