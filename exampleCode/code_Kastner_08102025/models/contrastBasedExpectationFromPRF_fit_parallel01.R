...
debugFlag <- 0
if ( debugFlag==1) {
  mainDir <- '/home/alessiof/data/KastnerContrastBasedTest'
  #mainDir <- '//wsl.localhost/Ubuntu-20.04/home/alessiof/data/KastnerContrastBasedTest'
  setwd(mainDir)
  args <- c('HMR28_kastnerFinerCw.nii.gz',
            'HMR28_pRF_params.nii.gz',
            'completeStimuliKastnerOrig_borderOnlyAfterSaccade_shifted_final.Rdata',
            'cw')
}
if ( debugFlag==0 ) {
  args <- commandArgs(T)
...
varExp <- pRFArray[,12]
selectedIdxAll <- which( varExp > 0.50 ) 
#selectedIdx <- selectedIdxAll

#split between chuncks for parallel fitting (about 400 voxels each)
...
modelFunction <- function( passIdx, tsArray, pRFArray, completeStimuliSequence, rescale_var, trValue, limitsSelVoxelsMatrix, selectedIdxAll ) {
  
  #print( sprintf('passIdx: %d out of %d ...', passIdx, dim( limitsSelVoxelsMatrix )[1] ) )
  #system( sprintf('passIdx: %d out of %d ...', passIdx, dim( limitsSelVoxelsMatrix )[1] ) )
  tempMessage <- sprintf('passIdx_%d_of_%d', passIdx, dim(limitsSelVoxelsMatrix)[1] )
  system( sprintf('echo %s', tempMessage) )  
  
  library( pracma )
  library( abind )
  library( neuRosim )
  
  selectedVoxelsPassTemp <- limitsSelVoxelsMatrix[ passIdx, 1 ] : limitsSelVoxelsMatrix[ passIdx, 2 ]
  ...
  outMat <- cbind( selectedIdx, outputR2, outputDetrendedTs, outputPredictedTs )
  return( outMat ) 
...
runParallel <- 1
if ( runParallel == 0 ){
  storeTimePar <- system.time( outputAll <- lapply( 1:4, modelFunction, 
                                                    tsArray=tsArray,
                                                    pRFArray=pRFArray,
...
if ( runParallel == 1 ){
  ...
  showConnections()
  storeTimePar <- system.time( outputAll <- parLapply( cl, 1:dim( limitsSelVoxelsMatrix )[1], 
                                                    modelFunction, 
                                                    tsArray=tsArray,
                                                    pRFArray=pRFArray,
                                                    completeStimuliSequence=completeStimuliSequence,
...
#rearrange output
outputLength <- length( outputAll )
outputMat <- outputAll[[1]]
for ( k in 2:outputLength ) {
  outputMat <- rbind( outputMat, outputAll[[k]] )  
}
selectedIdx <- outputMat[,1]
outputDetrendedTs <- outputMat[,3:164] 
outputPredictedTs <- outputMat[,165:326]
outputR2 <- outputMat[,2]
...
sprintf( 'writing file: %s ...', outputFileName )
tsArrayEmpty <- array( 0, c( dim( tsArray )[1], 1 ) )
tsArrayEmpty[ selectedIdx ] <- outputR2 
storeOriginalTs <- array( tsArrayEmpty, c( dim(tsFile$brk)[1:3] ) )
write.AFNI( outputFileName, brk = storeOriginalTs,
            origin = tsFile$origin, orient = tsFile$orient,
            defhead = tsFile$NI_head)
