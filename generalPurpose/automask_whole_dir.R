...
for (k in 1:length(filesCpDir)) {
  #if (k < 10) { instr <- sprintf('3dAutomask -apply_prefix 0%1.0f.nii.gz %s', k, filesCpDir[k] ) }
  #if (k >= 10) { instr <- sprintf('3dAutomask -apply_prefix %1.0f.nii.gz %s', k, filesCpDir[k] ) }
