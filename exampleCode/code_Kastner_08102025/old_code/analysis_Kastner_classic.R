...
for ( nSubjects in 1:length(anatomiesDir) ) { #subjId <- 5 length(anatomiesDir)
  ...
  CWmod <- read.AFNI('zzz_trWave05_CW.nii.gz')
  CCWmod <- read.AFNI('zzz_trWave05_CCW.nii.gz') # this is the volume (CCW) where right hemi phase values are [0,pi] and left hemi value are [pi,2pi], like the modelling results, so keep this volume as the base and shift the phase values of the other volume (CW)
  CWmodVol <- CWmod$brk
  ...
  CWmod <- read.AFNI('zzz_trWave10_CW.nii.gz')
  CCWmod <- read.AFNI('zzz_trWave10_CCW.nii.gz')
  CWmodVol <- CWmod$brk
  CCWmodVol <- CCWmod$brk
  outVol <- array(0, dim( CCWmod$brk ) )
  weightVolumeCW <- CWmodVol[,,,2]
