...
#DISTANCEROI='roiP.1D.roi'
#PARENTVOLUME='anatomy_N3.nii.gz'
#DEPTH ='depth.nii.gz'
#args <- c( BOUNDARYNAME, DISTANCEROI, PARENTVOLUME, DEPTH, '/usr/lib/afni/bin','/home/alessiofracasso/Dropbox/analysisAfni/surfaces')
...
instr <- sprintf( '%s/SurfDist -i_1D surfaces_folder/%s_sm.1D.coord surfaces_folder/%s_or.1D.topo -input datasetStart.1D > exampleStart.1D', args[5], args[1], args[1], startLine_nodes )
...
if ( sum( storeNodes=='NaN' )>0 ) {
  indxNan <- which( storeNodes=='NaN' )
  storeNodes[indxNan] <- 0
}
if ( sum( storeNodes=='NA' )>0 ) {
  indxNan <- which( storeNodes=='NA' )
  storeNodes[indxNan] <- 0
}
