for (nSubj in 1:5) {
  ...
  #### apply a posteriori non-lin correction ####
  resultsDir <- 'test_concatenated.results'
  if ( file.exists( sprintf('%s/vr_base_min_outlier_adjusted.nii', resultsDir ) ) ) { system( sprintf('rm %s/vr_base_min_outlier_adjusted.nii', resultsDir ) ) }
  if ( file.exists( sprintf('%s/vr_base_min_outlier_adjusted_concatenated.nii', 'coregistration' ) ) ) { system( sprintf('rm %s/vr_base_min_outlier_adjusted_concatenated.nii', 'coregistration' ) ) }
  if ( file.exists( sprintf('%s/stats_concatenated_ts.nii', anatomyDir ) ) ) { system( sprintf('rm %s/stats_concatenated_ts.nii', anatomyDir ) ) }
  if ( file.exists( sprintf('%s/stats_concatenated_ts_masked.nii', anatomyDir ) ) ) { system( sprintf('rm %s/stats_concatenated_ts_masked.nii', anatomyDir ) ) }
  ...
  if ( file.exists( sprintf('%s/anatCopy_resampled_to_stats.nii.gz', anatomyDir ) ) ) { system( sprintf('rm %s/anatCopy_resampled_to_stats.nii.gz', anatomyDir ) ) }
  if ( file.exists( sprintf('%s/meanTs_in_anatomy.nii', anatomyDir ) ) ) { system( sprintf('rm %s/meanTs_in_anatomy.nii', anatomyDir ) ) }
  if ( file.exists( sprintf('%s/stats_concatenated_REML_ts.nii', anatomyDir ) ) ) { system( sprintf('rm %s/stats_concatenated_REML_ts.nii', anatomyDir ) ) }
  if ( file.exists( sprintf('%s/stats_concatenated_REML_ts_masked.nii', anatomyDir ) ) ) { system( sprintf('rm %s/stats_concatenated_REML_ts_masked.nii', anatomyDir ) ) }
  ...
  fileToCorrect <- 'vr_base_min_outlier+orig'
  fileToCorrect_cleanName <- 'vr_base_min_outlier_adjusted'
  
  # apply non linear transform, to check, applied to the vr_base_min_outlier+orig file #
  instr <- paste('3dNwarpApply',
                 sprintf( '-master test_concatenated.results/%s', fileToCorrect ),
                 sprintf( '-source test_concatenated.results/%s', fileToCorrect ),
                 '-nwarp topUpDir/warpTop_PLUS_WARP+orig',
                 '-dxyz 2',
                 '-interp wsinc5',
                 '-short',
                 sprintf('-prefix test_concatenated.results/%s.nii', fileToCorrect_cleanName ) )
  print( instr )
  system( instr )
  
  # apply non linear transform to concatenated results, combined with coregistration in one single step, to check in the coregistration folder #
  instr <- paste('3dNwarpApply',
                 sprintf( '-master coregistration/%s', 'anatCopy.nii.gz' ),
                 sprintf( '-source test_concatenated.results/%s', fileToCorrect ),
                 '-nwarp coregistration/coregMat.1D topUpDir/warpTop_PLUS_WARP+orig',
                 '-dxyz 2',
                 '-interp wsinc5',
                 '-short',
                 sprintf('-prefix coregistration/%s_concatenated.nii', fileToCorrect_cleanName ) )
  print( instr )
  system( instr )
  
  # apply non linear transform concatenated with coregistration applied to stats #
  instr <- paste('3dNwarpApply',
                 sprintf( '-master coregistration/%s', 'anatCopy.nii.gz' ),
                 sprintf( '-source test_concatenated.results/stats.test_concatenated+orig' ),
                 '-nwarp coregistration/coregMat.1D topUpDir/warpTop_PLUS_WARP+orig',
                 '-dxyz 2',
                 '-interp NN',
                 sprintf('-prefix %s/stats_concatenated_ts.nii', anatomyDir ) )
  print( instr )
  system( instr )
  
  # apply non linear transform concatenated with coregistration applied to stats (REML) #
  instr <- paste('3dNwarpApply',
                 sprintf( '-master coregistration/%s', 'anatCopy.nii.gz' ),
                 sprintf( '-source test_concatenated.results/stats.test_concatenated_REML+orig' ),
                 '-nwarp coregistration/coregMat.1D topUpDir/warpTop_PLUS_WARP+orig',
                 '-dxyz 2',
                 '-interp NN',
                 sprintf('-prefix %s/stats_concatenated_REML_ts.nii', anatomyDir ) )
  print( instr )
  system( instr )
  
  # mask the stats on the anatomy only
  
  # resample anatomy to stats_concatenated_ts.nii resolution
  instr <- paste('3dresample',
                 sprintf('-input %s/anatCopy.nii.gz', anatomyDir),
                 sprintf('-master %s/stats_concatenated_ts.nii', anatomyDir),
                 sprintf('-prefix %s/anatCopy_resampled_to_stats.nii.gz', anatomyDir) )
  system( instr )
  
  # use resampled anatomy to mask stats
  instr <- paste('3dcalc',
                 sprintf('-a %s/anatCopy_resampled_to_stats.nii.gz', anatomyDir),
                 sprintf('-b %s/stats_concatenated_ts.nii', anatomyDir),
                 sprintf('-expr \u0027step(a)*b\u0027'),
                 sprintf('-prefix %s/stats_concatenated_ts_masked.nii', anatomyDir),
                 '-datum FLOAT' )
  system( instr )
  
  # use resampled anatomy to mask stats REML
  instr <- paste('3dcalc',
                 sprintf('-a %s/anatCopy_resampled_to_stats.nii.gz', anatomyDir),
                 sprintf('-b %s/stats_concatenated_REML_ts.nii', anatomyDir),
                 sprintf('-expr \u0027step(a)*b\u0027'),
                 sprintf('-prefix %s/stats_concatenated_REML_ts_masked.nii', anatomyDir),
                 '-datum FLOAT' )
  system( instr )
  ...
  #### bring mean TS into anatomy folder (these are the motion corrected, top-up mean ts. it is sufficient to use the affine coregistration) ####
  instr <- paste( '3dAllineate',
                  sprintf('-prefix %s/meanTs_in_anatomy.nii', anatomyDir),
                  '-1Dmatrix_apply coregistration/coregMat.1D',
                  '-final lin',
                  '-input meanTsFolder/meanTs.nii',
                  sprintf( '-master %s/anatCopy_resampled_to_stats.nii.gz', anatomyDir ) )
  system( instr )
