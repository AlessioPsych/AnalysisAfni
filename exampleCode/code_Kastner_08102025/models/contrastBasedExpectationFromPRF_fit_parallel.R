...
# run code without parallelization
runParallel <- 0
if ( runParallel == 0 ){
...
if ( runParallel == 1 ){
  ...
  showConnections()
  storeTimePar <- system.time( outputAll <- parLapply( cl, 1:4, modelFunction, 
                                                    tsArray=tsArray,
                                                    pRFArray=pRFArray,
                                                    completeStimuliSequence=completeStimuliSequence,
                                                    rescale_var=rescale_var,
...
#participant name
participantName <- strsplit( inputEyeEPI, '[_]' )[[1]][1]

#save original ts, to make sure everything is in the correct space and place
outputFileName <- sprintf( '%s_%s_originalTs_pRFBasedPrediction.nii.gz', participantName, inputDirection )
...
#save detrended ts
outputFileName <- sprintf( '%s_%s_detrendedTs_pRFBasedPrediction.nii.gz', participantName, inputDirection )
tsArrayEmpty <- array( 0, dim( tsArray ) )
...
#save predicted ts
outputFileName <- sprintf( '%s_%s_predictedTs_pRFBasedPrediction.nii.gz', participantName, inputDirection )
tsArrayEmpty <- array( 0, dim( tsArray ) )
...
#save r2 values
outputFileName <- sprintf( '%s_%s_R2_pRFBasedPrediction.nii.gz', participantName, inputDirection )
