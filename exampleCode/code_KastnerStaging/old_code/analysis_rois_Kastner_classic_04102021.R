...
#filenameOut <- 'testResults_all_in_Kastner_classic_withHrf_24052021.RData'
...
sMod <- summary( lmer( varExp ~ roi_sorted + ( 1 | subject ), data=size_ecc_df_base_globalAverage_var ) )
for (i in 1:length(sMod$coefficients[,5])) { 
  if ( sMod$coefficients[i,5] < 0.01 ) {
    if (i==1) { text( i, bp$stats[5,i]+0.1, '*', cex=3, col='red' ) }
    if (i!=1) { text( i, bp$stats[5,i]+0.1, '*', cex=3 ) }
  }
}

dev.copy2pdf( file='prf_varExp_adjR2.pdf', width=4.5, height=4.5 )
graphics.off()
...
##### all rois, kastner var explained and analysis, model 05 #####
x11(width=4.5, height=4.5)
...
sMod <- summary( lmer( adjR2_05 ~ roi_sorted + ( 1 | subject ), data=df_kastner_model_aggregate_var ) )
for (i in 1:length(sMod$coefficients[,5])) { 
  if ( sMod$coefficients[i,5] < 0.01 ) {
    if (i==1) { text( i, bp$stats[5,i]+0.1, '*', cex=3, col='red' ) }
    if (i!=1) { text( i, bp$stats[5,i]+0.1, '*', cex=3 ) }
...
dev.copy2pdf( file='kastner05_varExp_adjR2.pdf', width=4.5, height=4.5 )
graphics.off()




##### all rois, kappa par, model 05 #####
x11(width=4.5, height=4.5)
...
sMod <- summary( lmer( kappa ~ roi_sorted + ( 1 | subject ), data=df_kastner_model_aggregate_kappa ) )
for (i in 1:length(sMod$coefficients[,5])) { 
  if ( sMod$coefficients[i,5] < 0.01 ) {
    if (i==1) { text( i, bp$stats[5,i]+0.1, '*', cex=3, col='red' ) }
    if (i!=1) { text( i, bp$stats[5,i]+0.1, '*', cex=3 ) }
...
dev.copy2pdf( file='kastner05_kappaPar.pdf', width=4.5, height=4.5 )
graphics.off()
...
boxplot( df_kastner_model_aggregate_adjR2$adjR2_05 ~ df_kastner_model_aggregate_adjR2$roi_sorted, 
         add=FALSE, boxwex=0.3, ylim=c(0, 0.6), at = 1:14, xlim=c(0,15), yaxs='i', axes=TRUE, outline=FALSE, lty=1, col='gray50',
...
bp <- boxplot( df_kastner_model_aggregate_adjR2$adjR2_phase05 ~ df_kastner_model_aggregate_adjR2$roi_sorted, 
         add=TRUE, boxwex=0.3, at = 1:14+0.3, xlim=c(0,15), yaxs='i', frame=FALSE, axes=FALSE, outline=FALSE, lty=1, col='red' )
...
legend( 7.8, 0.12, c('model','phase encoded'), bty='n', fill=c('gray50','red') )
dev.copy2pdf( file='kastner05_modelComparison.pdf', width=4.5, height=4.5 )
graphics.off()
...
boxplot( df_kastner_model_aggregate_adjR2$adjR2_both ~ df_kastner_model_aggregate_adjR2$roi_sorted,
         add=FALSE, boxwex=0.2, ylim=c(0, 0.6), at = 1:14, xlim=c(0,15), yaxs='i', axes=TRUE, outline=FALSE, lty=1, col='gray50',
         frame=FALSE, las=2, cex.lab=1.35, cex.axis=1.25, xlab='ROI', ylab='adj. R^2 (proportion)' )
boxplot( df_kastner_model_aggregate_adjR2$adjR2_05 ~ df_kastner_model_aggregate_adjR2$roi_sorted,
         add=TRUE, frame=FALSE, las=1, boxwex=0.2, at = 1:14-0.25, xlim=c(0,15), yaxs='i', outline=FALSE, lty=1, col='lightblue', axes=FALSE )
boxplot( df_kastner_model_aggregate_adjR2$adjR2_phase05 ~ df_kastner_model_aggregate_adjR2$roi_sorted,
         add=TRUE, boxwex=0.2, at = 1:14+0.25, xlim=c(0,15), yaxs='i', frame=FALSE, axes=FALSE, outline=FALSE, lty=1, col='red' )
boxplot( df_kastner_model_aggregate_adjR2$adjR2_10 ~ df_kastner_model_aggregate_adjR2$roi_sorted,
         add=TRUE, boxwex=0.2, at = 1:14, xlim=c(0,15), yaxs='i', frame=FALSE, axes=FALSE, outline=FALSE, lty=1, col='orange' )
boxplot( df_kastner_model_aggregate_adjR2$adjR2_phase10 ~ df_kastner_model_aggregate_adjR2$roi_sorted,
         add=TRUE, boxwex=0.2, at = 1:14-0.25, xlim=c(0,15), yaxs='i', frame=FALSE, axes=FALSE, outline=FALSE, lty=1, col='blue' )
abline( v=seq(1.5,13.5), lwd=0.5, lty=2 )
...
dev.copy2pdf( file='kastner_full_modelComparison.pdf', width=9, height=4.5 )
graphics.off()
...
dev.copy2pdf( file='kastner05_10_ratio.pdf', width=4.5, height=4.5 )
graphics.off()
...
plotDirectionDistribution <- function( df_kastner_model_filt_clean, outputSuffix ) {
  ...
  for ( subjSel in unique( df_kastner_model_filt_clean$subject )  ) {
    ...
    dev.copy2pdf( file=sprintf('directionResultsModelling05/subject_%s_%s.pdf',subjSel,outputSuffix), width=9, height=2 )
    graphics.off()
  }
  ...
  levels( aggDFDensity$roiArray ) <- levels( aggDFDensity$roiArray )[c(4,5,3,1,2)]
  ...
  dev.copy2pdf( file=sprintf('directionResultsModelling05/allSubjects_%s.pdf',outputSuffix), width=9, height=2 )
  graphics.off()
...
dev.copy2pdf( file='exampleROC_curve.pdf', width=4.5, height=4.5 )
graphics.off()
...
rOut <- c()
aucOut <- c()
for ( s in levels( df_kastner_model_filt_clean$subj ) ) {
  for ( r in levels( df_kastner_model_filt_clean$roi_coarse )[1:5] ) {
    print( sprintf( 'subject: %s, roi: %s', s, r ) )
    dfLoop <- subset( df_kastner_model_filt_clean, subj==s & roi_coarse==r )
...
plotDirectionDistribution <- function( df_kastner_model_filt_clean, outputSuffix ) {
  ...
  for ( subjSel in unique( df_kastner_model_filt_clean$subject )  ) {
    ...
    dev.copy2pdf( file=sprintf('directionResultsPhase05/subject_%s_%s.pdf',subjSel,outputSuffix), width=9, height=2 )
    graphics.off()
  }
  ...
  dev.copy2pdf( file=sprintf('directionResultsPhase05/allSubjects_%s.pdf',outputSuffix), width=9, height=2 )
  graphics.off()
}
