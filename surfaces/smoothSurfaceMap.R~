...
for ( k in 1:length(surfaceFilesCoords) ) {
  ...
  outputFile <- sprintf( '%s/%s_smooth', newDir, outSplitName[[1]][1] )
  instr <- sprintf('SurfSmooth -spec surfaces_folder/%s -surf_A surfaces_folder/%s -met HEAT_07 -input %s%s -fwhm %s -output %s',
