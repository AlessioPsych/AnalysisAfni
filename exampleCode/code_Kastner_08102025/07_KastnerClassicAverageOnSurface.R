...
surfaceFolder <- '/media/alessiofracasso/DATADRIVE1/KastnerData/staging_area_Kastner/anatomies_KastnerClassic_Freesurfer/surfaceAtlases/suma_MNI152_2009/'
anatomyDir <- sprintf( '%s/anatomies_KastnerClassic_Freesurfer', mainFolder )
testSubjectDir <- '/media/alessiofracasso/DATADRIVE1/KastnerData/staging_area_Kastner/anatomies_KastnerClassic_Freesurfer/ASM16_ANATOMY/FreeSeg_result/SUMA/KastnerNewModelSurfaces1D_test/'
...
dirToCheck <- sprintf('%s/kastnerClassicAverageModelsSurfaces1D_hrf_correction_noBothModels', surfaceFolder )
...
for ( nAverages in 1:length( cleanFilenames ) ) { #length( cleanFilenames ) #nAverages <- 1
  ...
  write.table( x=round(fileStore,4), file=sprintf('%s/kastnerClassicAverageModelsSurfaces1D_hrf_correction_noBothModels/%s', surfaceFolder, fileNameOutput ), col.names = FALSE, row.names = FALSE )
