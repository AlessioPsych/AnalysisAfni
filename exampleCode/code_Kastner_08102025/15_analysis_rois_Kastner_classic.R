...
mainDir <- '/media/alessiofracasso/DATADRIVE1/KastnerData/staging_area_Kastner'
...
#### individual participant tuning location distribution an population vector decoding strategy ####
...
# ) 
df_kastner_model_filt_clean$dv <- (df_kastner_model_filt_clean$mu + pi/2) %% (2*pi)
tempAa <- subset( df_kastner_model_filt_clean, participant=='SCA01' & roi_coarse=='IPS0/IPS1' & side=='right' )
coordsPlot <- pol2cart( cbind( tempAa$dv, tempAa$kappa ) )
xPlot <- coordsPlot[,1]
yPlot <- coordsPlot[,2]
levels( df_kastner_model_filt_clean$roi_coarse )
levels( df_kastner_model_filt_clean$participant )
par(mfrow=c(2,2))
...
hist( tempAa$mu, xlab='radians', main='tuning location', xlim=c(0,2*pi) )
hist( tempAa$kappa, xlab='kappa', main='tuning width', xlim=c(0,1) )
hist( tempAa$adjR2_05, xlab='var. exp.', main='var. exp.', xlim=c(0,1) )

#### Wrapped Gaussian response using dwrappednormal() ####
neural_response <- function(theta, mu, sigma) {
  # dwrappednormal uses mu and sd on circular scale
  dwrappednormal(circular(theta), mu = circular(mu), rho = sigma)
}

# scale to range function
rescale_to_range <- function(x, new_min = 0, new_max = 1) {
  ...
  rescaled <- scaled * (new_max - new_min) + new_min
  return(rescaled)
}

#### Test directions (true saccade directions) and individual participant parameters ####
test_directions <- seq(0, 2 * pi, length.out = 100)

#### Population vector decoder ####
decode_population_vector <- function(pop, test_directions) { # pop <- region_B
  errors <- numeric(length(test_directions))
  decoded_angles <- numeric(length(test_directions))
  
  for (i in seq_along(test_directions)) { # i <- 18
    theta <- test_directions[i]
    
    # Compute response for each neuron to this test direction
    responses <- mapply(neural_response, rep( theta, dim(pop)[1] ), pop$tuning_location, pop$tuning_width)
    
    # Weight responses by goodness of fit
    responses <- responses * pop$goodness_of_fit
    
    # Compute vector sum
    x <- cos(pop$tuning_location) * responses
    y <- sin(pop$tuning_location) * responses
    
    sum_x <- sum(x)
    sum_y <- sum(y)
    decoded_angle <- atan2(sum_y, sum_x) %% (2 * pi)
    
    decoded_angles[i] <- decoded_angle
    errors[i] <- as.numeric(circular(decoded_angle - theta))
  }
...
df_kastner_model_filt_clean$dv <- (df_kastner_model_filt_clean$mu + pi/2) %% (2*pi)
tempAa <- subset( df_kastner_model_filt_clean, participant=='SCA01' & roi_coarse=='IPS0/IPS1' )
tempAa$tuning_location <- tempAa$mu 
tempAa$tuning_width <- tempAa$kappa
tempAa$goodness_of_fit <- tempAa$adjR2_05

#### Decode both populations ####
result_decoding <- decode_population_vector(tempAa, test_directions)

test_directions <- seq(0, 2 * pi, length.out = 50)
participantsId <- levels( df_kastner_model_filt_clean$participant )
roiId <-levels( df_kastner_model_filt_clean$roi_coarse )
errorMatOut <- array( 999, c( length( participantsId), length( roiId ) ) )
for ( partIdx in 1:length( participantsId ) ) {
  for ( rIdx in 1:length( roiId ) ) {
    print( sprintf( 'processing participant: %s, roi %s ...', participantsId[ partIdx ], roiId[ rIdx ] ) )
    df_kastner_model_filt_clean$dv <- (df_kastner_model_filt_clean$mu + pi/2) %% (2*pi)
    tempAa01 <- subset( df_kastner_model_filt_clean, participant==participantsId[ partIdx ] & roi_coarse==roiId[ rIdx ] )
    sampledIdxs <- sample( dim(tempAa01)[1], 50, replace=FALSE )
    tempAa <- tempAa01[ sampledIdxs, ]
    #tempAa <- subset( tempAa01, participant==participantsId[ partIdx ] & roi_coarse==roiId[ rIdx ] & adjR2_05 > quantile( tempAa01$adjR2_05, 0.15  ) )
    tempAa$tuning_location <- tempAa$mu 
    tempAa$tuning_width <- tempAa$kappa
    tempAa$goodness_of_fit <- tempAa$adjR2_05
    result_decoding <- decode_population_vector(tempAa, test_directions)
    errorMatOut[ partIdx, rIdx ] <-  round(result_decoding$mean_error, 3)
  }
}
boxplot( errorMatOut )
...
cat("Region A (broad tuning, good fit):", round(result_decoding$mean_error, 3), "\n")
cat("Region B (narrow tuning, poor fit):", round(result_decoding_V1$mean_error, 3), "\n")
