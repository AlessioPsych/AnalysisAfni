...
if [ -z "$1" ]
then
echo 'segment script using nighres (MPRAGE)'
echo 'Inputs:'
echo 'MPRAGE, input anatomy'
echo 'example call: segmentScript_mprageSkullStripped.sh mprage.nii.gz'
exit 1
fi

3dresample -orient RAI -prefix _del_MP2RAGE_RAI.nii.gz -inset $MPRAGE -rmode NN
3dAutobox -input _del_MP2RAGE_RAI.nii.gz -noclust -prefix MP2RAGE_RAI_SKULL_STRIP_BOX.nii.gz -npad 5 
segment_white_matter_params_no_ants.sh MP2RAGE_RAI_SKULL_STRIP_BOX.nii.gz 0.0001 0.001 5 7-7-7-7-7-7 0 1 1 
segment_gray_matter.sh MP2RAGE_RAI_SKULL_STRIP_BOX.nii.gz white_matter_mask.nii.gz 0.65-0.65-0.65-0.65
