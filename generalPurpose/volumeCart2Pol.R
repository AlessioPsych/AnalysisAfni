...
volumeCart2Pol <- function( inputVol, xIdx, yIdx, labelOut  ) {
  instr <- sprintf( '3dcalc -a %s[%s] -b %s[%s] -expr \u0027 atan2(a,b) \u0027 -prefix del_ThetaAfterBlur.nii.gz', inputVol, xIdx, inputVol, yIdx ); system( instr )
  instr <- sprintf( '3dcalc -a %s[%s] -b %s[%s] -expr \u0027 sqrt(a^2+b^2) \u0027 -prefix del_RhoAfterBlur.nii.gz', inputVol, xIdx, inputVol,  yIdx ); system( instr )
  instr <- sprintf( '3dTcat -prefix %s %s del_ThetaAfterBlur.nii.gz del_RhoAfterBlur.nii.gz', labelOut, inputVol ); system( instr )
  ...
  instr <- sprintf( '3drefit -sublabel %1.0f rho_conv %s', (nVols+1), labelOut ) ; system( instr ) 
  system('rm _ttt_nvols.1D del_ThetaAfterBlur.nii.gz del_RhoAfterBlur.nii.gz')
}
