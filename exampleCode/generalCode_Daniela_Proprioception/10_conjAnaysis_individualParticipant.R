...
for (nSubj in 1:6) { #nSubj <- 1
  ...
  maskQuantileThr <- 0.75
  contrastQuantileThr <- 0.90
  
  fileStats <- 'stats_concatenated_ts_masked.nii'
  statsFile <- read.AFNI( fileStats )
  statsVolumes <- statsFile$brk
  statsVolume_active_rest <- statsVolumes[,,,18]
  statsVolume_passive_rest <- statsVolumes[,,,21]
  statsVolume_touch_rest <- statsVolumes[,,,24]
  maskThrValue <- round( quantile( statsVolume_touch_rest[ abs( statsVolume_touch_rest ) > 10^-9 ], probs = c( maskQuantileThr ) ), 2 )
  activeThrValue <- round( quantile( statsVolume_active_rest[ abs( statsVolume_active_rest ) > 10^-9 ], probs = c( contrastQuantileThr ) ), 2 )
  passiveThrValue <- round( quantile( statsVolume_passive_rest[ abs( statsVolume_passive_rest ) > 10^-9 ], probs = c( contrastQuantileThr ) ), 2 )
  
  if ( file.exists( sprintf( 'tOutcome_mask_asInPaper2022.nii.gz' ) ) ) {
    instr <- sprintf( 'rm tOutcome_mask_asInPaper2022.nii.gz' ); system( instr )
  } 
  ...
  instr <- paste( '3dcalc',
                  sprintf('-a %s\u027[23]\u027', fileStats ),
                  sprintf( '-expr \u027 not( step(a-%1.4f) ) \u027', maskThrValue ),
                  '-prefix tOutcome_mask_asInPaper2022.nii.gz' )
  print( instr )
  system( instr )
  
  instr <- paste( '3dcalc',
                  sprintf('-a %s\u027[17]\u027', fileStats ),
                  sprintf('-b %s\u027[20]\u027', fileStats ),
                  '-c tOutcome_mask_asInPaper2022.nii.gz',
                  sprintf( '-expr \u027 step(a-%1.4f)*c + step(b-%1.4f)*c \u027', activeThrValue, passiveThrValue ),
                  '-prefix tOutcome_linear_conjunction_asInPaper2022.nii.gz' )
  print( instr )
  system( instr )
  
  
  fileStats <- 'stats_concatenated_REML_ts_masked.nii'
  statsFile <- read.AFNI( fileStats )
  statsVolumes <- statsFile$brk
  statsVolume_active_rest <- statsVolumes[,,,18]
  statsVolume_passive_rest <- statsVolumes[,,,21]
  ...
  if ( file.exists( sprintf( 'tOutcome_mask_asInPaper2022_REML.nii.gz' ) ) ) {
    instr <- sprintf( 'rm tOutcome_mask_asInPaper2022_REML.nii.gz' ); system( instr )
  } 
  
  if ( file.exists( sprintf( 'tOutcome_linear_conjunction_asInPaper2022_REML.nii.gz' ) ) ) {
    instr <- sprintf( 'rm tOutcome_linear_conjunction_asInPaper2022_REML.nii.gz' ); system( instr )
  } 
  
  instr <- paste( '3dcalc',
                  sprintf('-a %s\u027[23]\u027', fileStats ),
                  sprintf( '-expr \u027 not( step(a-%1.4f) ) \u027', maskThrValue ),
                  '-prefix tOutcome_mask_asInPaper2022_REML.nii.gz' )
  print( instr )
  system( instr )
  ...
  instr <- paste( '3dcalc',
                  sprintf('-a %s\u027[17]\u027', fileStats ),
                  sprintf('-b %s\u027[20]\u027', fileStats ),
                  '-c tOutcome_mask_asInPaper2022_REML.nii.gz',
                  sprintf( '-expr \u027 step(a-%1.4f)*c + step(b-%1.4f)*c \u027', activeThrValue, passiveThrValue ),
                  '-prefix tOutcome_linear_conjunction_asInPaper2022_REML.nii.gz' )
  print( instr )
  system( instr )
