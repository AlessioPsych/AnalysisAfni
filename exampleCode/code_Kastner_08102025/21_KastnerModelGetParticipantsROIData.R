...
paramsFileKastnerModel <- list.files( path=getwd(), pattern='*params*', recursive = FALSE, full.names = FALSE  )

allParamsTemp <- c( paramsFileKastnerModel )
allParams <- c()
...
setwd( resultsFolder )
dirToCheck <- sprintf('KastnerModel' )
...
for ( nModels in 1:length( allParams ) ) { # nModels <- 1
  ...
  for ( nPart in 1:length( modelParticipants )  ) { # nPart <- 1 # length( modelParticipants )
    ...
    setwd( 'roisWang2015_KastnerModel' ) # use rois from pRF data, however all the EPIS are coregistered to the same anatomy, so we can compare between models
    print('########')
    print( getwd() )
    print('########')
  ...
  save( outModelDf, file = sprintf( '%s/%s/%s.RData', resultsFolder, 'KastnerModel', allParams[ nModels ] ) )
}
