...
#setwd('/home/alessiof/testData/testSeg_nighres101_MP2RAGE_HSA')
#args <- c('roiDepthDel.nii.gz', 'profilesOut_profiles.nii.gz', 'modelTest08','3')
...
profilesMat <- round( scaleData( profilesMat, 8000, 1000 ) )
...
runModel <- function(runIndex) { #run all models on a single profile
...
write.AFNI( predictedProfilesName, brk = storeParameterProfiles,
            origin = inputProfilesFile$origin, orient = inputProfilesFile$orient,
            defhead = inputProfilesFile$NI_head)
labels <- letters[ 1:dim(storeParameterProfiles)[4] ]
labels[1] <- 'intercept'
labels[ length(labels) ] <- 'r2'
