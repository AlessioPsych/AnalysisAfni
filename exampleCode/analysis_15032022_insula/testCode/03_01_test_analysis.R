...
#for (nFile in 1:length(filesList) ) {
  ...
  #profilesVolume <- profilesFile$brk
  #depthVolume <- depthFile$brk[,,,1]
  #roiIdx <- which( roiVolume>0.01 & roiVolume < 1 & depthVolume>0.4  & depthVolume<0.6 )
  ...
  kMacroVoxels <- kmeans(x=dataMat, centers=1000, nstart=10) 
  str( dataMat )
  ...
  str( aggregateCoords )
  p <- plot_ly( aggregateCoords, x = ~xc, y = ~yc, z = ~zc, color = ~cluster ) %>%
    add_markers() %>%
    layout(scene = list(xaxis = list(title = 'xc'),
                        yaxis = list(title = 'yc'),
                        zaxis = list(title = 'zc')))
  ...
  p <- plot_ly( aggregateCoords[ modelMacroVoxels[2,] < 0,  ], x = ~xc, y = ~yc, z = ~zc, size=1 ) %>%
    add_markers() %>%
    layout(scene = list(xaxis = list(title = 'xc'),
                        yaxis = list(title = 'yc'),
                        zaxis = list(title = 'zc')))
  ...
  #####################################################################
  keepMacroVoxel <- rep(TRUE,dim(modelMacroVoxels)[2])#modelMacroVoxels[2,] < 0
  ...
  #data_umap <- umap( aggregateProfilesMat, scale=FALSE, n_neighbors=10, min_dist = 0.001, metric='cosine'  )
  #kUmap <- kmeans( data_umap, centers = 2, nstart = 5 )
  
  #prProfiles <- prcomp( aggregateProfilesMat )
  #kUmap <- kmeans( prProfiles$x[,c(1,2,3,4)], centers = 3, nstart = 5 )
  
  kUmap <- kmeans( aggregateProfilesMat_01, centers = 2, nstart = 2500,  algorithm = c("Hartigan-Wong" ), iter.max=1500 )
  
  #for (nK in 1:50) {
  #  kUmapTemp <- kmeans( scale( aggregateProfilesMat ), centers = 2, nstart = 25,  algorithm = c("Hartigan-Wong" ), iter.max=150 )
  #  if (nK==1) { kUmapOut <- kUmapTemp$cluster }
  #  if (nK>1) { kUmapOut <- ( kUmapTemp$cluster + kUmapOut )/2 }
  #}
  #kUmap$cluster <- kUmapOut
  #kHClust <- hclust( dist( aggregateProfilesMat, method='maximum' ) )
  #plot( kHClust )
  #clusterCut <- cutree(kHClust, 2); table( clusterCut )
  #table( clusterCut )
  #kUmap$cluster <- clusterCut
  
  #library( randomForest )
  #profiles.urf <- randomForest( aggregateProfilesMat )
  #MDSplot(profiles.urf)
  #plot( profiles.urf$proximity[,1], profiles.urf$proximity[,2] )
  ...
  # dev.off()
  # 
  # storeProfiles01[ nFile, ] <- profile01
