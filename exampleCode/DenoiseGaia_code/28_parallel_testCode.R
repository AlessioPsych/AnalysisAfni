...
print(args[1])
var<- args[1]
for ( nSubj in var : var  ) {# length( singleSubjectFolders ) # nSubj <- 1
  ...
  for ( nFilesToProcess in 1 : length( nifti_files_toProcess ) ) { # nFilesToProcess <- 1
    ...
    instr <- sprintf( 'afni 3dAutomask -clfrac 0.2 -apply_prefix %s %s',
    ...
    # denoise instruction
    instr <- sprintf('dwidenoise -nthreads 28 %s %s/%s/func/%s', 
                     inputToDenoiseAutomask, 
                     copyDir, 
