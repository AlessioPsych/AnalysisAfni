...
if (debugFlag==1) {
  ...
  outputFileName <- 'results_Wang_ORIG_Denoised_incongruent_congruent.RData'
...
getResultsGlasser <- function( mainFolder, inputFolder, outputFolder, outputFileName, roiFilenameLH_input, roiFilenameRH_input, MNI_flag ) {
  ...
  statFilename <- 'stats_RT_correct_basis_no.'
  tsnrFilename <- 'TSNR.'
  ...
  outputDataset <- data.frame( voxelValue=double(), 
    ...
    fileToSave <- sprintf('%s/%s/%s_%s', mainDir, outputDir, partNameClean, outputFileName )
...
outputFileName <- 'results_Glasser_ORIG_Denoised_singleVoxel_stats_RT_correct_basis_no.RData'
