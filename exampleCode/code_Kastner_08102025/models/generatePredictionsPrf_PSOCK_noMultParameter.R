...
print('get stimulus...')
setwd('/media/alessiofracasso/DATADRIVE1/KastnerData/staging_area_Kastner/code/stims/')
if (stimType==1) { 
...
if (fineFit==0) {
  xElements <- 14 #22
  yElements <- 16 #24
  sigmaArrayPositiveElements <- 15 #32
  multParElements <- 0 #5
...
if (dim( predictionGridGlobal )[1] <= 300 ) {
  totalIterations <- 3
}
if (dim( predictionGridGlobal )[1] > 300 ) {
  totalIterations <- ceil( dim( predictionGridGlobal )[1]/250 )
}
