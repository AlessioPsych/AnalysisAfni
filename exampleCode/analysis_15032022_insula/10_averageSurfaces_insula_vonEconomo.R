...
saveDir <- '/analyse/Project0165/Fracasso_Anatomy/AHEAD_database/10007840/results/insulaSurfaceFromVolumetricAnalysis_vonEconomo_poly01'
...
for (freesurferDirCounter in 1:2) {
  ...
  for ( i in 1:length( participantsFolders ) ) { #i <- 1
    ...
    loadFileNameLh <- sprintf('insulaClusters_vonEconomo_run_%s_lh.1D.dset', filenameIn ) 
    loadFileNameRh <- sprintf('insulaClusters_vonEconomo_run_%s_rh.1D.dset', filenameIn ) 
    
    instr <- sprintf('cp %s %s/%s_insulaClusters_vonEconomo_run_lh.1D.dset', loadFileNameLh, saveDir, participantsFolders[i] ); system( instr )
    instr <- sprintf('cp %s %s/%s_insulaClusters_vonEconomo_run_rh.1D.dset', loadFileNameRh, saveDir, participantsFolders[i] ); system( instr )
...
# left hemisphere
setwd( saveDir )
surfaceFile <- dir( pattern = '*_run_lh.1D.dset*')
...
write.table( surfaceData, file=sprintf( '%s/average_insulaClusters_%s_vonEconomo_corrected_lh.1D.dset', atlasDir, filenameIn ), row.names=FALSE, col.names=FALSE )

# right hemisphere
setwd( saveDir )
surfaceFile <- dir( pattern = '*_run_rh.1D.dset*')
...
write.table( surfaceData, file=sprintf( '%s/average_insulaClusters_%s_vonEconomo_corrected_rh.1D.dset', atlasDir, filenameIn ), row.names=FALSE, col.names=FALSE )
...
for (group in AgeGroups){
  ...
  sprintf("Processing Group: %s",group_name)
  selectedParticipants = group
  
  # left hemisphere
  setwd( saveDir )
  surfaceFile <- dir( pattern = '*run_lh.1D.dset*')
  surfaceFile_thickness <- dir( pattern = '*lh.thickness_ppp_*')
  surfaceFile_curvature <- dir( pattern = '*lh.curv_ppp_*')
  ...
  surfaceData[,10] <- averageLH_thickness 
  surfaceData[,11] <- averageLH_curvature
  #write.table( surfaceData, file=sprintf( '%s/average_insulaClusters_%s_vonEconomo_selectedParticipants_corrected_lh.1D.dset', atlasDir, filenameIn ), row.names=FALSE, col.names=FALSE )
  write.table( surfaceData, file=sprintf( '%s/average_insulaClusters_%s_vonEconomo_%s_lh.1D.dset', atlasDir, filenameIn , group_name ), row.names=FALSE, col.names=FALSE )
  
  # right hemisphere
  setwd( saveDir )
  surfaceFile <- dir( pattern = '*run_rh.1D.dset*')
  surfaceFile_thickness <- dir( pattern = '*rh.thickness_ppp_*')
  surfaceFile_curvature <- dir( pattern = '*rh.curv_ppp_*')
  storeSingleSubjectMap <- array( 9999, c( 198811, length( selectedParticipants ) ) )
  storeSingleSubjectMap_01 <- array( 9999, c( 198811, length( selectedParticipants ) ) )
  ...
  surfaceData[,10] <- averageRH_thickness 
  surfaceData[,11] <- averageRH_curvature
  #write.table( surfaceData, file=sprintf( '%s/average_insulaClusters_%s_vonEconomo_selectedParticipants_corrected_rh.1D.dset', atlasDir, filenameIn ), row.names=FALSE, col.names=FALSE )
  write.table( surfaceData, file=sprintf( '%s/average_insulaClusters_%s_vonEconomo_%s_rh.1D.dset', atlasDir, filenameIn , group_name ), row.names=FALSE, col.names=FALSE )
}