...
if [ -z "$1" ]
 then
  ...
  echo 'can be 4D; interpolation is NN, you must have the file spec.surfaces.smoothed'
  ...
  echo 'SURFACESDIR=$2, surfaces directory' 
  exit 1
fi
...
Rscript $AFNI_TOOLBOXDIRSURFACES/interpolateSurface.R $VOLUME $SURFACESDIR
