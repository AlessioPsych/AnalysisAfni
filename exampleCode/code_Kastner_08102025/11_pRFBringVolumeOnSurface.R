...
for ( nPart in 1:length( modelParticipants )  ) { # nPart <- 2 # length( modelParticipants )
  ...
  print( getwd() )
  print('#####')
  pRFModel <- list.files( getwd(), patter='params', full.names = FALSE, recursive = FALSE)
  allModels <- c( pRFModel )
  ...
  print( getwd() )
  print('#####')
  dirToCheck <- sprintf('pRFModelAnatomy' )
  ...
  print( getwd() )
  print('#####')
  dirToCheck <- sprintf('pRFModelSurfacesNiml' )
  if ( dir.exists( dirToCheck ) ) {
    instr <- sprintf( 'rm -R %s', dirToCheck ) 
    print( instr )
    if (runCodeFlag==1) { system( instr ) }  
  ...
  print( getwd() )
  print('#####')
  dirToCheck <- sprintf('pRFModelSurfaces1D' )
  if ( dir.exists( dirToCheck ) ) {
    instr <- sprintf( 'rm -R %s', dirToCheck ) 
    print( instr )
    if (runCodeFlag==1) { system( instr ) }  
  ...
  for ( nModels in 1:length( allModels ) ) {
    instr <- paste('cp',
                   ...
                   sprintf('%s/%s/FreeSeg_result/SUMA/pRFModelAnatomy/%s', anatomyDir, selectedAnatFolders[ nPart ], allModels[ nModels ] )
    )
    print( instr )
    if (runCodeFlag==1) { system( instr ) }  
  }
  ...
  for ( nModels in 1:length( allModels ) ) {
    for (outName in c('niml','1D') ) { # outName <- 'niml'
      for (nHemi in c('lh','rh') ) { # nHemi <- 'lh'
        ...
        if (outName=='niml') {
          modelOutputNameInstruction <- sprintf( '-out_niml %s/%s.%s.%s.dset', 'pRFModelSurfacesNiml', modelOutputName, nHemi, outName ) 
        }
        if (outName=='1D') {
          modelOutputNameInstruction <- sprintf( '-out_1D %s/%s.%s.%s.dset', 'pRFModelSurfaces1D', modelOutputName, nHemi, outName ) 
        }  
